 ...
 * 
 *  rg/pkg/freeswan/klips/net/ipsec/alg/ipsec_alg_3des_twinpass.c
 *
...
extern int _des_twinpass_set_key(struct ipsec_alg_enc *alg,__u8 *key_e,
    const __u8 * key, size_t keysize);
extern int _3des_twinpass_cbc_encrypt(struct ipsec_alg_enc *alg, __u8 * key_e,
...
#define ESP_3DES_KEY_SZ	        (3*8)	
#define ESP_3DES_CBC_BLKLEN     8

static struct ipsec_alg_enc ipsec_alg_3DES_twinpass = {
    ixt_version:	IPSEC_ALG_VERSION,
    ixt_module:		THIS_MODULE,
    ixt_refcnt:		ATOMIC_INIT(0),
    ixt_name:		"3des_twinpass",
    ...
    ixt_e_set_key:	_des_twinpass_set_key,
    ixt_e_cbc_encrypt:	_3des_twinpass_cbc_encrypt,
};
...
IPSEC_ALG_MODULE_INIT(ipsec_3des_init)
{
    ...
    if (esp_id)
	ipsec_alg_3DES_twinpass.ixt_alg_id = esp_id;
    if (excl)
	ipsec_alg_3DES_twinpass.ixt_state |= IPSEC_ALG_ST_EXCL;
    ret = register_ipsec_alg_enc(&ipsec_alg_3DES_twinpass);
    if (debug)
    {
	ipsec_log("ipsec_3des_init(alg_type=%d alg_id=%d name=%s): ret=%d\n",
	    ipsec_alg_3DES_twinpass.ixt_alg_type,
	    ipsec_alg_3DES_twinpass.ixt_alg_id,
	    ipsec_alg_3DES_twinpass.ixt_name,
	    ret);
    }
    if (ret==0 && test)
    {
...
{
    unregister_ipsec_alg_enc(&ipsec_alg_3DES_twinpass);
    return;
}
