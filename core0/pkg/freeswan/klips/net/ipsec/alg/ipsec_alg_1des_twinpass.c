 ...
 * 
 *  rg/pkg/freeswan/klips/net/ipsec/alg/ipsec_alg_1des_twinpass.c
 *
...
extern int _1des_twinpass_cbc_encrypt(struct ipsec_alg_enc *alg, __u8 * key_e,
...
#define ESP_DES_CBC_BLKLEN      8

static struct ipsec_alg_enc ipsec_alg_1DES_twinpass = {
    ixt_version:	IPSEC_ALG_VERSION,
    ixt_module:		THIS_MODULE,
    ixt_refcnt:		ATOMIC_INIT(0),
    ixt_name:		"1des_twinpass",
    ...
    ixt_e_cbc_encrypt:	_1des_twinpass_cbc_encrypt,
};
...
IPSEC_ALG_MODULE_INIT(ipsec_1des_init)
{
    ...
    if (esp_id)
	ipsec_alg_1DES_twinpass.ixt_alg_id = esp_id;
    if (excl)
	ipsec_alg_1DES_twinpass.ixt_state |= IPSEC_ALG_ST_EXCL;
    ret = register_ipsec_alg_enc(&ipsec_alg_1DES_twinpass);
    if (debug)
    {
	ipsec_log("ipsec_1des_init(alg_type=%d alg_id=%d name=%s): ret=%d\n",
	    ipsec_alg_1DES_twinpass.ixt_alg_type,
	    ipsec_alg_1DES_twinpass.ixt_alg_id,
	    ipsec_alg_1DES_twinpass.ixt_name,
	    ret);
    }
    if (ret==0 && test)
    {
...
{
    unregister_ipsec_alg_enc(&ipsec_alg_1DES_twinpass);
    return;
}
