 ...
 * 
 *  rg/pkg/freeswan/klips/net/ipsec/alg/ipsec_alg_aes_twinpass.c
 *
...
extern int _aes_twinpass_set_key(struct ipsec_alg_enc *alg, __u8 * key_e, 
    const __u8 * key, size_t keysize);
extern int _aes_twinpass_cbc_encrypt(struct ipsec_alg_enc *alg, __u8 * key_e,
...
typedef struct {
    ...
    uint8_t    aes_key[ESP_AES_KEY_SZ_MAX];  /* the encryption key schedule */
} aes_context;
...
static struct ipsec_alg_enc ipsec_alg_AES = {
    ...
    ixt_e_set_key:	_aes_twinpass_set_key,
    ixt_e_cbc_encrypt:	_aes_twinpass_cbc_encrypt,
};
