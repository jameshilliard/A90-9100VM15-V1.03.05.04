 ...
 * Version: 0.8.1
 *
 * $Id: ipsec_alg.c,v 1.3.16.1 2006/07/18 09:56:52 sergey Exp $
 *
...
#define barf_out(fmt, args...)  do { ipsec_log(KERN_ERR "%s: (%s) " fmt, __FUNCTION__, ixt->ixt_name , ## args)\
	; goto out; } while(0)
...
static int ipsec_alg_insert(struct ipsec_alg *ixt) {
	...
	if (ixt->ixt_list.next != &ixt->ixt_list || 
		...
		ipsec_log(KERN_ERR "ipsec_alg_insert: ixt object \"%s\" "
...
int ipsec_alg_auth_key_create(struct ipsec_sa *sa_p) {
	...
	{
		sa_p->ips_key_a = ixt_a->ixt_a_hmac_new_key(ixt_a, akp, sa_p->ips_key_bits_a/8);
		ret = sa_p->ips_key_a ? 0 : -EINVAL;
	}
	else
	{
		if ((sa_p->ips_key_a = 
...
int register_ipsec_alg(struct ipsec_alg *ixt) {
	...
	if (ret==0) {
		...
		ipsec_log(KERN_ERR "pfkey_list_insert_supported returned %d. "
				"Loading anyway.\n", ret);
	ret=0;
out:
	return ret;
...
static int ipsec_alg_test_auth(int auth_alg, int test) {
	...
	if (ixt_a->ixt_a_hmac_new_key)
	{
		tmp_key_a = ixt_a->ixt_a_hmac_new_key(ixt_a, test_key, keysize);
		ret = test_key_a ? 0 : -EINVAL;
	}
	else 
	{
		tmp_key_a = test_key_a;
	...
	ret=ixt_a->ixt_a_hmac_hash(ixt_a, tmp_key_a, test_auth, BUFSZ, test_hash, AHHMAC_HASHLEN);
	ipsec_log(KERN_INFO
		    ...
		    "ret=%d\n", ret);
	{
		...
		for (i = 0; i < 5; i++) {
			...
			while (jiffies == now) {
				mb();
				ixt_a->ixt_a_hmac_hash(ixt_a, tmp_key_a, test_auth, BUFSZ, test_hash, AHHMAC_HASHLEN);
				mb();
				count++;
				mb();
			}
...
out:
	if (tmp_key_a && ixt_a->ixt_a_hmac_destroy_key) ixt_a->ixt_a_hmac_destroy_key(ixt_a, tmp_key_a);
	if (buf) kfree(buf);
...
int ipsec_alg_test(unsigned alg_type, unsigned alg_id, int test) {
	...
	ipsec_log(KERN_ERR "klips_info: ipsec_alg_test() called incorrectly: "
			"alg_type=%d alg_id=%d\n",
			alg_type, alg_id);
	return -EINVAL;
}
