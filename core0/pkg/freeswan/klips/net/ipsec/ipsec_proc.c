	...
	for (i = 0; i < SADB_HASHMOD; i++) {
		for (sa_p = ipsec_sadb_hash[i];
...
#ifdef CONFIG_IPSEC_NAT_TRAVERSAL
			{
				...
				len += snprintf(buffer + len, length-len, " natencap=%s",
					       natttype_name);
				
				len += snprintf(buffer + len, length-len, " natsport=%d",
					       sa_p->ips_natt_sport);
				
				len += snprintf(buffer + len,length-len, " natdport=%d",
					       sa_p->ips_natt_dport);
			}
#else
			len += snprintf(buffer + len, length-len, " natencap=na");
		     ...
		     int length     IPSEC_PROC_LAST_ARG)
{
	...
	for(cur=ipsec_dev_head; cur; cur=cur->next) {
		dev = ipsec_dev_get(cur->ipsec_dev->name);
		if(dev) {
			  ...
			  int length      IPSEC_PROC_LAST_ARG)
{
	...
	len += sprintf(buffer + len, "debug_ipcomp=%08x.\n", sysctl_ipsec_debug_ipcomp);
	len += sprintf(buffer + len, "debug_verbose=%08x.\n", sysctl_ipsec_debug_verbose);
	len += sprintf(buffer + len, "debug_reject=%08x.\n", debug_reject);
	len += sprintf(buffer + len, "debug_log_all=%08x.\n", debug_log_all);
...
struct proc_dir_entry ipsec_natt =
{
	0,
	13, "ipsec_natt",
	S_IFREG | S_IRUGO, 1, 0, 0, 0,
...
#  else /* !PROC_FS_2325 */
	...
	proc_net_create ("ipsec_natt", 0, ipsec_natt_get_info);
#    ifdef CONFIG_IPSEC_DEBUG
...
#    ifdef CONFIG_IPSEC_DEBUG
	if (proc_net_unregister(ipsec_klipsdebug.low_ino) != 0)
		ipsec_log("klips_debug:ipsec_cleanup: "
 ...
 * NOTIFY: automerge
 *
 * Revision 1.7.2.1  2006/02/23 17:32:13  sergey
 * B6059: implement (OpenSWAN based) IPSec NAT-Traversal.
 *
 * Revision 1.7  2005/11/14 10:54:29  sergey
 * AUTO MERGE: 1 <- branch-4_1
 * B26997: enable IPSec with linux 2.6.
 * NOTIFY: automerge
 *
 * Revision 1.6.162.1  2005/11/14 10:47:20  sergey
 * B26997: enable IPSec with linux 2.6.
 *
 * Revision 1.6  2004/01/18 16:36:47  sergey
 * FIX: B7835. Added rate limit to IPSec log.
 *
 * Revision 1.5  2003/09/21 20:23:15  igork
 * merge branch-dev-2421 into dev
 *
 * Revision 1.4  2003/09/21 15:13:30  sergey
 * FIX: B5670. Added log for rejected IPSec packets (ICSA).
 *
 * Revision 1.2.16.1  2003/09/16 13:34:20  ron
 ...
 * NOTIFY: automerge
 *
 * Revision 1.2.18.1  2003/09/11 11:34:41  yoavp
 * R7234: add NONE to range-types, remove RCSIDs from freeswan and some cosmetics
 *
 * Revision 1.2  2003/05/26 19:01:27  eyalm
 * Merge branch-2_5-start to tag-5_2_2 into dev
 *
 * Revision 1.1  2003/02/19 11:46:31  sergey
 * Initial revision
 *
