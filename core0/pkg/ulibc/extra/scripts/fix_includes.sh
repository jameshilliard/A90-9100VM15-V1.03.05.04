...
if [ ! -f "$BUILDDIR/os/linux/Makefile" -a ! -f "$KERNEL_SOURCE/include/linux/version.h" ]; then
    echo "";
    echo "";
    echo "The file $BUILDDIR/os/linux/Makefile or $KERNEL_SOURCE/include/linux/version.h is missing!";
...
if [ -f "$BUILDDIR/os/linux/Makefile" ] ; then
# set current VERSION, PATCHLEVEL, SUBLEVEL, EXTRAVERSION
eval `sed -n -e 's/^\([A-Z]*\) = \([0-9]*\)$/\1=\2/p' -e 's/^\([A-Z]*\) = \(-[-a-z0-9]*\)$/\1=\2/p' $BUILDDIR/os/linux//Makefile`
else
