...
#ident "$Id: insmod.c,v 1.4.2.2 2006/06/27 06:22:30 itay Exp $"
...
static int obj_relocate(struct obj_file *f, ElfW(Addr) base, char *m_name)
{
	...
	if (!cCONFIG_RG_INSMOD_SILENT)
	{
		...
		for (i = 0; i < n; ++i)
		{
			...
			else if (!strcmp(secname, ".main_flow"))
				main_flow = f->sections[i]->header.sh_addr;
		}

		if (text)
			printf("insmod: add-symbol-file build/debug/%s.o %#lx", m_name, text);
		else
			printf("insmod: no text section for module %s", m_name);
		
		if (main_flow)
			printf(" -s .main_flow %#lx", main_flow);
		if (data)
