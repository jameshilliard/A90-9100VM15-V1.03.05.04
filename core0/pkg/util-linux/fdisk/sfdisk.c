...
#ifndef CONFIG_PPC /* prevent warning message for ppc compiler */
static
#endif
...
msdos_signature (struct sector *s) {
    if (*(unsigned short *) (s->data + 0x1fe) != MSDOS_MAGIC) {
...
typedef struct { unsigned char h,s,c; } __attribute__ ((packed)) chs; /* has some c bits in s */
chs zero_chs = { 0,0,0 };

typedef struct { unsigned long h,s,c; } __attribute__ ((packed)) longchs;
...
write_partitions(char *dev, int fd, struct disk_desc *z) {
	...
	*(unsigned short *)(&(s->data[0x1fe])) = MSDOS_MAGIC;
    }
    if (save_sector_file) {
