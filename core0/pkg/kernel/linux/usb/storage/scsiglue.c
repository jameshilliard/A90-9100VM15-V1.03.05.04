 ...
 *
 * $Id: scsiglue.c,v 1.3.2.1 2006/08/06 15:46:49 itay Exp $
 *
...
{
	static char host_serial[100];
	struct us_data *us = (struct us_data *)host->hostdata[0];
	char serial[40] = {};

	if (us)
	    sprintf(serial, ";SERIAL=%s", us->serial);

	sprintf(host_serial, "SCSI emulation for USB Mass Storage devices%s",
	    serial);
