...
if [ "y" = "$CONFIG_WSTL_4HOMEMEDIA_HOMECONTROL" ] ; then
echo "Copying $BUILDDIR/../4hm/* to $CRAMFS_DIR/"
cp -r $BUILDDIR/../4hm/* $CRAMFS_DIR/
cd $RAMDISK_RW_DIR
mkdir -p usr
cd usr
ln -sf /mnt/cramfs/usr/lib lib
cd $RAMDISK_RW_DIR/lib
ln -sf /mnt/cramfs/lib/zi zi
ln -sf /mnt/cramfs/lib/libcore.so libcore.so
ln -sf /mnt/cramfs/lib/foundation.jar foundation.jar
cd ../etc
ln -sf /mnt/cramfs/etc/zwavebridge zwavebridge
cd ../bin
ln -sf /mnt/cramfs/bin/cvm cvm
ln -sf /mnt/cramfs/bin/core core
ln -sf /mnt/cramfs/bin/zwavebridge zwavebridge
fi
...
if [ "" != "CONFIG_ATHEROS_ART_TOOLS" ] ; then
  $RG_MKNOD dk c 63 0
  $RG_MKNOD dk0 c 63 0
fi
...
if [ "y" != "$CONFIG_SIMPLE_CRAMDISK" ] ; then
    ...
    if [ "y" = "$CONFIG_RG_MODFS" ] ; then
      if [ "y" = "$CONFIG_RG_MODFS_ROMFS" ] ; then
        ...
        $BUILDDIR/pkg/cramfs/mkcramfs -b $CRAMFS_BLKSZ \
...
if [ "y" = "$CONFIG_CRAMFS_FS" ] ; then
	...
	$BUILDDIR/pkg/cramfs/mkcramfs -e $TARGET_ENDIANESS_L \
    ...
    if [ "y" = "$CONFIG_SIMPLE_CRAMDISK" ] ; then
	$BUILDDIR/pkg/cramfs/mkcramfs $RAMDISK_RW_DIR \
	    $RAMDISK_IMG_GZ
    fi
