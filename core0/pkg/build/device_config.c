...
code2str_t dev_if_type_cfg_str[] = {
    ...
    { DEV_IF_AR521X, "DEV_IF_AR521X" },
    { DEV_IF_AR521X_VAP, "DEV_IF_AR521X_VAP" },
    { DEV_IF_TWINPASS_ETH, "DEV_IF_TWINPASS_ETH" },
    { DEV_IF_MV88E60XX_HW_SWITCH, "DEV_IF_MV88E60XX_HW_SWITCH" },
    { DEV_IF_MV88E60XX_HW_WAN_SWITCH, "DEV_IF_MV88E60XX_HW_WAN_SWITCH" },
    { DEV_IF_MOCA_EN2210_MII, "DEV_IF_MOCA_EN2210_MII" },
    { DEV_IF_IKANOS_VDSL, "DEV_IF_IKANOS_VDSL" },
    { DEV_IF_PSB6973_HW_SWITCH, "DEV_IF_PSB6973_HW_SWITCH" },
    { DEV_IF_PSB6973_HW_WAN_SWITCH, "DEV_IF_PSB6973_HW_WAN_SWITCH" },
    { DEV_IF_AR50WL, "DEV_IF_AR50WL" },
    { DEV_IF_AR50WL_VAP, "DEV_IF_AR50WL_VAP" },
    { -1, NULL }
};
    ...
    logical_network_t net)
{
    ...
    /*Comcast wants wireless as default off*/
    if (token_get("CONFIG_WSTL_COMCAST") && type == DEV_IF_AR521X_VAP) {
         fprintf(dev, "dev/%s/%s 0\n", name, Senabled);
    }
    else
...
void dev_moca_conf_set(char *dev_name, char *mii_device)
{
    fprintf(dev, "dev/%s/%s/%s %s\n", dev_name, Smoca, Smii_device, mii_device);
}

void dev_hw_switch_hide_port(char *dev_name, int port)
{
    fprintf(dev, "dev/%s/%s/%s/%d/%s 1\n", dev_name, Shw_switch, Sports, port,
	Senabled);
    fprintf(dev, "dev/%s/%s/%s/%d/%s 1\n", dev_name, Shw_switch, Sports, port,
	Sis_hidden);
}

void dev_hw_switch_force_port(char *dev_name, int port)
{
    fprintf(dev, Sdev "/%s/" Shw_switch "/" Sforce_port " %d\n", dev_name,
	port);
}

void dev_add_description(char *dev_name, char *desc)
{
    fprintf(dev, "dev/%s/%s %s \n", dev_name, Sdescription, desc);
}
