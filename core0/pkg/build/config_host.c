...
#define NEEDED_PPC_TOOLCHAIN_VER 20060801
...
static void conf_endian_flags(void)
{
	...
	else if (token_get("CONFIG_CPU_MIPS32") || token_get("CONFIG_CPU_LX4189"))
	{
...
static void conf_tools_prefix(void)
{
	...
	if (token_get("CONFIG_CX8620X") || token_get("CONFIG_COMCERTO") ||
	    token_get("CONFIG_KS8695") || token_get("CONFIG_ARCH_SOLOS"))
	{
	    str_catprintf(&TOOLS_PREFIX, "%s/%s-2", TOOLCHAIN_ROOT,
		token_get_str("TARGET_PREFIX"));
	}

	if (token_get("CONFIG_INFINEON_TWINPASS"))
	{
	    str_catprintf(&TOOLS_PREFIX, "%s/%s-eb", TOOLCHAIN_ROOT,
		token_get_str("TARGET_PREFIX"));
	}
    ...
    if (stat(token_get_str("TOOLS_PREFIX"), &st))
	conf_err("Cannot find toolchain %s\n", token_get_str("TOOLS_PREFIX"));
	...
	int tc_ver = atoi(sys_get(NULL, *str_printf(&tc_ver_cmd,
	    "cat %s/version", token_get_str("TOOLS_PREFIX"))));

	if (tc_ver < NEEDED_PPC_TOOLCHAIN_VER)
	{
	    conf_err("Installed powerpc toolchain version is %d, needed %d\n",
		tc_ver, NEEDED_PPC_TOOLCHAIN_VER);
	}
	str_free(&tc_ver_cmd);
    }

    token_set("CROSS_COMPILE", "%s/bin/%s-", token_get_str("TOOLS_PREFIX"),
	token_get_str("TARGET_PREFIX"));
}
...
static char *libc_detect(char **libc_path)
{
	...
	for (major = libc->major_from; major <= libc->major_to; major++)
	{
		...
		for (build = libc->build_from; build <= libc->build_to; build++)
		{
		    ...
		    str_printf(libc_path, "%s/%s/lib/%s%d.%d.%d.so",
			TOOLS_PREFIX, token_get_str("TARGET_MACHINE"),
			libc->prefix, major, minor, build);

		    if (!lstat(*libc_path, &st))
			return libc->token;
		}
	    }
	}
...
static void conf_host_general(void)
{
	...
	{
	    rg_error(LNOTICE, "Could not find valgrind executable. "
...
static void conf_target_general(void)
{
    ...
    str_catprintf(&PATH, ":%s:%s/bin",
	token_get_str("CONFIG_RG_EXTERNAL_TOOLS_PATH") ? : "",TOOLS_PREFIX);
}
...
static void conf_rg_linux_cflags(void)
{
    ...
    if (token_get("CONFIG_RG_DEV") && !token_get("CONFIG_RG_JPKG"))
	str_catprintf(&f, "-g ");
	...
	str_catprintf(&flags_26, "-I%s/os/linux-2.6/arch/%s/include ", BUILDDIR, arch);
	...
	if (token_get("CONFIG_BCM963XX"))
	{
	    str_catprintf(&flags_26, "-I%s/os/linux-2.6/include/asm/mach-bcm963xx ",
		BUILDDIR);
	    str_catprintf(&flags_26, "-I%s/os/linux-2.6/include2/asm/mach-generic ",
		BUILDDIR);
	}
...
void conf_doctools(void)
{
    int rg_doc_enable = 0;
    struct stat st;
    char *doc_makefile = NULL, *latex_exe = sys_get(NULL,
    ...
    const char *filename)
{
	    ...
	    filename)))
	{
	    if (first_loc)
	    {
		rg_error(LWARN, "warning: %s found in both %s and in %s. "
		    "using %s\n", filename, first_loc, search_locations[i], 
		    first_loc);
		break;
	    }
	    else
...
static void conf_mysql(void)
{
	...
	{
	    fprintf(stderr, 
		"Compilation for this distribution requires installation of 'mysql'\n");
