...
#if 0 /* our glibc/ulibc has lseek64() which is replacing _llseek(), we use it */
...
setup_tables (void)
{
  ...
  if ((fat = (unsigned char *) malloc (FAT_SIZE_IN_USE)) == NULL)
    die ("unable to allocate space for FAT image in memory");

  memset( fat, 0, FAT_SIZE_IN_USE );
...
#define ZBUFS_PER_SYNC 64
static char zbuf[ZBUF_SIZE];

#define writezbuf(size,errstr)				\
  do {							\
    int __size = (size);				\
    do { \
      int to_write = ZBUF_SIZE > __size ? __size : ZBUF_SIZE; \
      writebuf(zbuf, to_write, errstr); \
      if (!(((size) - __size) % (ZBUFS_PER_SYNC * ZBUF_SIZE)) && \
	  fsync(dev)) 					\
      { \
	  error("fsync failed whilst writing " errstr); \
      } \
      __size -= to_write; \
    } while (__size > 0); \
  } while(0)
...
write_tables (void)
{
  ...
  for (x = 1; x <= nr_fats; x++)
  {
    writebuf( fat, FAT_SIZE_IN_USE, "FAT" );
    writezbuf( (fat_length * sector_size) - FAT_SIZE_IN_USE, "FAT" );
  }
