...
#define OFFSET_ABOVE_16MB 0 /* was defined in kernel .h, 0 for CPU0 */
#ifndef CONFIG_TWINPASS_64_MEG
#define	CPU1_BASE_ADDR	(KSEG1 | OFFSET_ABOVE_16MB | 0x01400000)	//OFFSET_ABOVE_20MB=0x01400000
#else
#define	CPU1_BASE_ADDR	(KSEG1 | OFFSET_ABOVE_16MB | 0x03400000)	//OFFSET_ABOVE_52MB=0x05200000
#endif
...
CPU1_Argument_T	*pCPU1_arguments = (CPU1_Argument_T *)CPU1_BASE_ADDR;
#endif
//prochao-,=======================================================
...
#else
// prochao, 7-24-2006, for dual-core support, the CPU0 uses only the 1st 20MB
#ifndef CONFIG_TWINPASS_64_MEG
	memsz = 20;		//20MB
#else
    memsz = 52;
...
#else
        add_memory_region(0x00000000, 0x4000000, BOOT_MEM_RAM);	//prochao+, 10/29/2006, cover all memory size (64MB)
#endif
...
// prochao, 7-24-2006, for dual-core support, the CPU0 uses only the 1st 16MB
	memsz = 16;		//16MB, the plus 1 is for the substraction at next line
#if	defined(CONFIG_DANUBE) || defined(CONFIG_BOARD_TWINPASS_E)
