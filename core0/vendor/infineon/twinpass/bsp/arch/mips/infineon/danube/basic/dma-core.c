...
** 12 Dec 2007 Peng LIU		Independant number of descriptor for each channel
**				TWEAK_DMA_BUFFER_RING_SIZE
** 23 Jan 2007  Xu Liang        In function "rx_chan_intr_handler", risk
**                              condition takes place between interrupt status
**                              is clean and g_danube_dma_int_status is clean,
**                              to fix it, check if descriptor is valid after
...
#if defined(CONFIG_DANUBE_PPA_D4)
  #define ENABLE_D4                     1
#endif
...
//			printk ("62");
		...
		*DANUBE_DMA_CS = chan_num;
		if (*DANUBE_DMA_CS != chan_num)
            printk(__FILE__ ":%d:%s: *DANUBE_DMA_CS (%d) != chan_num (%d)", __LINE__, __FUNCTION__, *DANUBE_DMA_CS, chan_num);
		*DANUBE_DMA_CIE = 0;	/*fix me, should disable all the interrupts here? */
		*DANUBE_DMA_IRNEN &= ~(1 << chan_num);	/*disable interrupts */
		*DANUBE_DMA_CCTRL &= ~1;
		while (*DANUBE_DMA_CCTRL & 1) {
...
map_dma_chan (_dma_chan_map * map)
{
  ...
  #if defined(ENABLE_D4) && ENABLE_D4
            && map[i].irq != DANUBE_DMA_CH6_INT
  #endif
		   )
            {
...
#else
            *DANUBE_DMA_PCTRL |= 0xf << 8;
            *DANUBE_DMA_PCTRL &= ~(1 << 6); //  disable dma drop
#endif
		}
...
/******************************************************************************/
//  proc for debug
static struct proc_dir_entry *g_sw_proc_dir = NULL;

static int proc_write_reg(struct file *file, const char *buf, unsigned long count, void *data)
{
    char str[64];
    char *p;
    ...
    if ( ch >= 0 && ch <= 7 )
    {
        ...
        local_irq_save(flag);
        *DANUBE_DMA_CS = ch;
        regs[0] = *DANUBE_DMA_CTRL;
        regs[1] = *DANUBE_DMA_IRNEN;
        regs[2] = *DANUBE_DMA_IRNCR;
        regs[3] = *DANUBE_DMA_IRNICR;
        regs[4] = *DANUBE_DMA_CS;
        regs[5] = *DANUBE_DMA_CCTRL;
        regs[6] = *DANUBE_DMA_CDBA;
        regs[7] = *DANUBE_DMA_CDLEN;
        regs[8] = *DANUBE_DMA_CIS;
        regs[9] = *DANUBE_DMA_CIE;
        local_irq_restore(flag);

        printk("DANUBE_DMA_CTRL   = %08X\n", regs[0]);
        printk("DANUBE_DMA_IRNEN  = %08X\n", regs[1]);
        printk("DANUBE_DMA_IRNCR  = %08X\n", regs[2]);
        printk("DANUBE_DMA_IRNICR = %08X\n", regs[3]);
        printk("ch %d:\n", regs[4]);
        printk("  DANUBE_DMA_CCTRL = %08X\n", regs[5]);
        printk("  DANUBE_DMA_CDBA  = %08X\n", regs[6]);
        printk("  DANUBE_DMA_CDLEN = %08X\n", regs[7]);
        printk("  DANUBE_DMA_CIS   = %08X\n", regs[8]);
        printk("  DANUBE_DMA_CIE   = %08X\n", regs[9]);
    }
...
static inline void proc_file_create(void)
{
    ...
    g_sw_proc_dir = proc_mkdir("sw", NULL);

    res = create_proc_read_entry("reg",
                                  0,
                                  g_sw_proc_dir,
                                  NULL,
                                  NULL);
    if ( res )
        res->write_proc = proc_write_reg;
}

static inline void proc_file_delete(void)
{
    remove_proc_entry("desc",
                      g_sw_proc_dir);
