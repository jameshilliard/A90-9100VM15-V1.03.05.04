...
#define MPS_BOOK_KEEPING_THRESH		0	//	0
...
#else
//prochao-
extern volatile MPS_BUFF_POOL_HEAD	* volatile *mps_buff_pool_array;
#endif
...
int32_t	mps_queue_init(volatile MPS_BUFF_HEAD *q, uint32_t size, uint32_t num_entries, MPS_ALLOC_FN alloc_fn)
{
	...
	tmp_circular_buf = mps_inv_alloc((num_entries + 1) * sizeof (MPS_BUF *));
	DPRINTK("q->circular_buf [0x%x] \n", (uint32_t)tmp_circular_buf);
	if (tmp_circular_buf == NULL) {
	...
	if (size) {
...
#endif
		{
			...
			tmp_circular_buf[i] = (MPS_BUF *)(*alloc_fn)(size);
			if (tmp_circular_buf[i] == NULL) {
...
#endif
			MPS_FLUSH_MPS_BUF(tmp_circular_buf[i]);
		}
		q->wpos = num_entries;
	} else {
		memset((void *)tmp_circular_buf, 0x0, sizeof(MPS_BUF *) * (num_entries+1));
		q->wpos = 0;
		q->rpos = 0;
	}
	q->circular_buf = tmp_circular_buf;
...
int32_t ifx_mps_buff_pool_init(u8 mps_service_seq_num, u8 sizes, uint32_t def_freeq_len)
{
	...
	mps_buf_pool = mps_inv_alloc(sizeof(*mps_buf_pool));
...
static struct sk_buff *mps_dev_alloc_skb_kernel(unsigned int length)
{
...
int32_t ifx_mps_queue_init(u8 mps_service_seq_num, uint32_t size, int num_entries, int32_t isfreeq)
{
	...
	if (!isfreeq) {
		...
		if (!q) {
			...
			memset((void *)q, 0x0, sizeof(*q));
			DPRINTK(" In [%s] free_q [0x%x]\n", __FUNCTION__, (uint32_t)q);
			mps_buf_size_entry->free_q = q;
		}
	}
	wmb();
	DEBUG_FNEXIT();
	return (mps_queue_init(q, size, num_entries, (MPS_ALLOC_FN)mps_dev_alloc_skb_kernel));
}
...
int32_t ifx_mps_is_mps_buffer(MPS_BUF  volatile *buf)
{
	...
	if (phyaddr < CPU1_RAM_PHYADDR_BEGIN || phyaddr > CPU1_RAM_PHYADDR_END)
	{
