...
struct dwc3884_channel {
  ...
  struct dwc3884_channel_regs *regs;
	// holds the status of the channel on the last interrupt 
	u32 status;	
...
static inline struct endpoint_status* 
endpoint_status(struct usb_device *dev, int pipe)
{
	struct hcd_dev *hcdev = (struct hcd_dev*)dev->hcpriv;
	int epnum = usb_pipeendpoint (pipe);
	if (usb_pipein (pipe) && !usb_pipecontrol (pipe))
		epnum |= 0x10;
	
	struct endpoint_status *epsts = (struct endpoint_status*)hcdev->ep[epnum];
	if(!epsts) {
		dbg("endpoint_status init for ep %d/%d", usb_pipedevice(pipe), usb_pipeendpoint(epnum));
		hcdev->ep[epnum] = kmalloc(sizeof(*epsts), GFP_ATOMIC);
		if(hcdev->ep[epnum]) {
			memset(hcdev->ep[epnum], 0, sizeof(*epsts));
		}
		epsts = (struct endpoint_status*)hcdev->ep[epnum];
	}
	return epsts;
}
...
#define CHAN_ENABLE   		(1<<CHAN_ENABLE_BIT)
	...
	u32 splt;
#define CHAN_SPLIT_BIT 		31
#define CHAN_SPLIT 				(1<<CHAN_SPLIT_BIT)
#define CHAN_CSPLIT_BIT		16	
#define CHAN_CSPLIT				(1<<CHAN_CSPLIT_BIT)
#define CHAN_XACTPOS(v) 	(((v)>>14)&0x3)
#define CHAN_HUB(v) 			(((v)>>7)&0x7f)
#define CHAN_PORT(v) 			(((v)>>0)&0x7f)	
	u32 interrupt;
	u32 intmsk;
...
#endif
#define dbg(format, arg...) printk(__FILE__ ":%d: " format "\n" ,\
		__LINE__, ## arg)

// very verbose debug, impacts the timing quite much...
#if CHAN_DEBUG
#define chan_dbg(chan, format, arg...) printk(__FILE__ ": ch%d " format "\n" ,\
		chan->index, ## arg)
#else
#define chan_dbg(chan, format, arg...)
#endif
