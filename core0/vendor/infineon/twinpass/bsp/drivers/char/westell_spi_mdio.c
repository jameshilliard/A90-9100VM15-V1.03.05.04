...
//	      - slave functionality
/* History
 * Last changed on: Oct, 2004
 * Last changed by: peng.liu@infineon.com
 * Reason: integration with Vinetic
 */
...
#define RESET_DEVICE  ((volatile u16*)0xb4a00000)
...
static int maj = 0;
static u32 local_tx_buf[100];
static u32 local_rx_buf[100];
...
static void kernel_free_buf(u32 *);
/************************************************************************
...
*/
        ...
        if (info->txbuf_ptr >= info->txbuf_end) {
                ...
                kernel_free_buf((u32 *)info->txbuf);
 //               kfree(info->txbuf);
                info->txbuf = NULL;
...
static void kernel_free_buf(u32 *buf_ptr)
{
    if (buf_ptr != (u32 *)&local_tx_buf && buf_ptr != (u32 *)&local_rx_buf) 
    {
        // should not get here
//        DANUBE_SSC_EMSG("kernel_free_buf called before device opened\n");
//        kfree((const void *)buf_ptr);
    }
}
...
*/
	...
	// free txbuf
	if (info->txbuf != NULL) {
        if (info->txbuf != (u32 *)&local_tx_buf) 
            printk("free problem in abort \n");
...
ifx_ssc_rxtx_mode_set(struct ifx_ssc_port *info, unsigned int val)
{
	...
	if (!(info) || (val & ~(IFX_SSC_MODE_MASK)))
		return -EINVAL;
    if (info->opts.modeRxTx  != val) 
    {    /*check BUSY and RXCNT*/	
	    while(  READ_PERIPHERAL_REGISTER(info->mapbase + IFX_SSC_STATE) & IFX_SSC_STATE_BUSY
	        ||READ_PERIPHERAL_REGISTER(info->mapbase + IFX_SSC_RXCNT) & IFX_SSC_RXCNT_TODO_MASK)
        {
            tmp++;
            udelay(5);
            if(tmp == 20)return -EBUSY;
        }
...
static void ifx_gpio_init(void) 
{
	...
	*(DANUBE_GPIO_P1_DIR) = ((*DANUBE_GPIO_P1_DIR)|(0x6))&(~1);
	*(DANUBE_GPIO_P1_ALTSEL0) = ((*DANUBE_GPIO_P1_ALTSEL0)|(0x7));
	*(DANUBE_GPIO_P1_ALTSEL1) = (*DANUBE_GPIO_P1_ALTSEL1)&(~0x7);
	*(DANUBE_GPIO_P1_OD) = (*DANUBE_GPIO_P1_OD)|0x0006;	

	/*CS3 */
	/*TODO: CS4 CS5 CS6*/
    *DANUBE_GPIO_P0_OUT = ( (*DANUBE_GPIO_P0_OUT) | 0x8000);

//    printk("spi:  gpio p1 dir %x \n",*DANUBE_GPIO_P1_DIR);
//    printk("spi:  gpio p1 as0 %x \n",*DANUBE_GPIO_P1_ALTSEL0);
//    printk("spi:  gpio p1 as1 %x \n",*DANUBE_GPIO_P1_ALTSEL1);
//    printk("spi:  gpio p1 od %x \n",*DANUBE_GPIO_P1_OD);
//    printk("spi:  gpio p0 out %x \n",*DANUBE_GPIO_P0_OUT);
...
{
	struct ifx_ssc_port *info;
    struct mii_ioctl_data *mdio_s;
    struct ifreq *pif = (struct ifreq *)data; 
	int line, ret_val = 0;
	unsigned long flags;
	unsigned long tmp;
    u32 mdio_w_data;
    ...
    case WESTELL_SSC_BAUD_SET:
        ...
        case WESTELL_SSC_BAUD_GET:
                ...
                break;
	        case WESTELL_SSC_RXTX_MODE_SET:
                if (from_kernel)
                ...
                break;
	        case WESTELL_SSC_RXTX_MODE_GET:
                tmp = READ_PERIPHERAL_REGISTER(info->mapbase + IFX_SSC_CON) &
	        ...
	        case WESTELL_SSC_ABORT:
                ifx_ssc_abort(info);
                break;
            case WESTELL_SSC_HWOPTS_GET:
...
{
#if 1
    printk("Westell (MDIO-SPI) driver\n"
...
/*
	i = maj;
*/
	if ((i = register_chrdev(maj, "wmdio", &ifx_ssc_fops)) < 0)
	{
	...
	for (i = 0; i < PORT_CNT; i++) {
		...
		info->opts.gpoCs = 0; // westell change was IFX_SSC_DEF_GPO_CS
 ...
 */
static int ssc_session(char * tx_buf, u32 tx_len, char * rx_buf, u32 rx_len, u16 PLD_mask)
{
	int ret=0;
    int busycount = 0;
    u32 preamble = 0xffffffff;
	volatile char * ssc_tx_buf=NULL;
    struct ifx_ssc_port *info;
	volatile char* ssc_rx_buf=NULL;
    info = &isp[0];
    u16 Mdio_Channel = PLD_mask;

    while(  READ_PERIPHERAL_REGISTER(info->mapbase + IFX_SSC_STATE) & IFX_SSC_STATE_BUSY
        ||READ_PERIPHERAL_REGISTER(info->mapbase + IFX_SSC_RXCNT) & IFX_SSC_RXCNT_TODO_MASK)
    {
//        printk("spi fifo busy \n");
        busycount++;
        udelay(10);
        if(busycount == 10)return -EBUSY;
    }
#ifdef DUAL_SPI_MDIO
    *(u16 *)0xB4800000 = Mdio_Channel;     // set Mdio Port
#endif
...
//	volatile char ssc_rx_buf[128]={0};
    ...
    while (info->txbuf != NULL && busycount < 10)
    {
        busycount++;
        udelay(10);
    }
    ...
    if (ifx_ssc_rxtx_mode_set(info, IFX_SSC_MODE_TX) < 0)
    {
        printk("SPI_MDIO: could not set TX mode \n");
        return -EBUSY;
    }
    ssc_tx_buf = (char*)&local_tx_buf; // kmalloc(sizeof(char) *  ((eff_size + 3) & (~3)), GFP_ATOMIC);
    memcpy(ssc_tx_buf,(char *)&preamble,4);
    ret=ifx_ssc_kwrite(0, ssc_tx_buf,4);

	if (rx_buf != NULL && rx_len != 0){
			mode = IFX_SSC_MODE_RX;
            ssc_rx_buf = (char*)&local_rx_buf; // kmalloc(sizeof(char) *  ((eff_size + 3) & (~3)), GFP_ATOMIC);
            if (ifx_ssc_rxtx_mode_set(info, IFX_SSC_MODE_RXTX) < 0)
            {
                printk("SPI_MDIO: could not set TX/RX mode \n");
                return -EBUSY;
            }
	...
	/* change by TaiCheng */
	//if (in_irq()){
    // Westell use static buffer to remove kmalloc overhead 
//	if (1){
//		ssc_tx_buf = (char*)&local_tx_buf; // kmalloc(sizeof(char) *  ((eff_size + 3) & (~3)), GFP_ATOMIC);
//		ssc_rx_buf = (char*)&local_rx_buf; // kmalloc(sizeof(char) *  ((eff_size + 3) & (~3)), GFP_ATOMIC);
//	}else{
...
//	memset((void*)ssc_tx_buf, 0, eff_size);
    ...
    {
        memcpy((char *)&ssc_tx_buf[0], tx_buf, 4);
        if ((ret=ifx_ssc_kwrite(0, ssc_tx_buf,4)) > 0)
              ssc_tx_buf = NULL;
    }
    else if (mode == IFX_SSC_MODE_RX){
        busycount = 0;
        memcpy(ssc_tx_buf, tx_buf, 4);
        if ((ret=ifx_ssc_kwrite(0, ssc_tx_buf,4)) > 0)
        {
            ssc_tx_buf = NULL;
        }
        if (  ret != 4 ){
		printk("ifx_ssc_write count %d\n",ret); 
		goto ssc_session_exit;
	}
//        *(unsigned long *)0xBE100B10 &= ~(0x8000);  // enable counter on for receive mdio
 //       *(unsigned long *)0xBE100B10 &= ~(0x8000);  // enable counter on for receive mdio
 //       ret=ifx_ssc_kread(0, ssc_rx_buf,eff_size);
 //       *(unsigned long *)0xBE100B10 |= 0x8000;     // disable counter for receive mdio
#ifdef DUAL_SPI_MDIO
        *(u16 *)0xB4800000 = (Mdio_Channel | 4);    // Enable PLD read
#else
        *(unsigned long *)0xBE100B10 &= ~(0x8000);  // enable counter on for receive mdio
#endif
        ret=ifx_ssc_kread(0, ssc_rx_buf,eff_size);
#ifdef DUAL_SPI_MDIO
        *(u16 *)0xB4800000 = PLD_mask;  //disable PLD read counter
#else
        *(unsigned long *)0xBE100B10 |= 0x8000;     // disable counter for receive mdio
#endif
        if (  ret != eff_size ){
        ...
        memcpy(rx_buf, (char *)&ssc_rx_buf[0], 4);
        ret = 4;
...
ssc_session_exit:
	...
	if (info->txbuf != NULL && ssc_tx_buf != NULL)
    {
        if (info->txbuf != (u32 *)&local_tx_buf) // This should not happen in Westell implementation
            DANUBE_SSC_EMSG("txbuf problem in session tx\n");

    }
    if (info->rxbuf != NULL && ssc_rx_buf != NULL)
    {
        if (info->rxbuf != (u32 *)&local_rx_buf) 
            DANUBE_SSC_EMSG("rxbuf problem in session tx\n"); // This should not happen in Westell implementation
 ...
 *	2. RX session
 */
int ifx_ssc_txrx(char * tx_buf, u32 tx_len, char * rx_buf, u32 rx_len, u16 PLD_mask)
{
	return ssc_session(tx_buf,tx_len,rx_buf,rx_len, PLD_mask);
}
 ...
 *		<0 error code
 */
int ifx_ssc_tx(char * tx_buf, u32 tx_len, u16 PLD_mask)
{
	return ssc_session(tx_buf,tx_len,NULL,0,PLD_mask);
}
 ...
 *		<0 error code
 */
int ifx_ssc_rx(char * tx_buf, u32 tx_len, char * rx_buf, u32 rx_len, u16 PLD_mask)
{
	return ssc_session(tx_buf,tx_len,rx_buf,rx_len,PLD_mask);
}
