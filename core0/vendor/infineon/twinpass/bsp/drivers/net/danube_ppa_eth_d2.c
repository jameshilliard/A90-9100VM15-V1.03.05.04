...
#include <asm/danube/westell_spi_mdio_defines.h>
#include <asm/danube/westell_spi_mdio.h>
...
//  specific board related configuration
   ...
   #ifdef CONFIG_WESTELL
    #undef  MII0_MODE_SETUP
    #define MII0_MODE_SETUP                     MII_MODE
   #endif
...
#define MARVEL_SWITCH_RESET                     ((volatile u16*)0xb4a00000)
...
#define MDIO_READ 0xC0000000

#define MDIO_WRITE_CMD(phy, reg, val) \
    (MDIO_WRITE | ((u32)(phy) << 21) | ((u32)(reg) << 16) | (u32)(val))

#define MDIO_READ_CMD(phy, reg) \
    (MDIO_READ | ((u32)(phy) << 21) | ((u32)(reg) << 16))

static u16 mdio_read(u16 phy, u16 reg)
{
    u32 val;

    while (*ETOP_MDIO_ACC & MDIO_BUSY);
    *ETOP_MDIO_ACC = MDIO_READ_CMD(phy, reg);
    while ((val = *ETOP_MDIO_ACC) & MDIO_BUSY);

#ifdef DEBUG_MDIO
    printk("mdio: read: phy=%x, reg=%x, val=%x\n", phy, reg, val & 0xffff);
#endif
    
    return val & 0xffff;
}

static void mdio_write(u16 phy, u16 reg, u16 val)
{
    while (*ETOP_MDIO_ACC & MDIO_BUSY);
    *ETOP_MDIO_ACC = MDIO_WRITE_CMD(phy, reg, val);
    /* while (*ETOP_MDIO_ACC & 0x80000000); */

#ifdef DEBUG_MDIO
    printk("mdio: write: phy=%x, reg=%x, val=%x\n", phy, reg, val);
#endif
}
...
/* Westell SPI/MDIO special funcions */
static INLINE int westell_spi_mdio_read(u32 mdio_register,u16 PLD_mask);
static INLINE int westell_spi_mdio_write(u32 mdio_register,u16 PLD_mask);
static INLINE int westell_spi_mdio_reset(int port, int flag);

void ppe_set_enable_pause(int set_enabled); // cr15452 enable/disable flow control on vdsl2 in bridge mode.

static int proc_write_stren(struct file *file, const char *buf, unsigned long count, void *data);
...
#endif
static int proc_read_spi(char *, char **, off_t, int, int *, void *);
static int proc_write_spi(struct file *, const char *, unsigned long, void *);
static int proc_write_mdio(struct file *, const char *, unsigned long, void *);
...
#ifdef CONFIG_WSTL_LOOP_DETECT
/* For Westell WSTL Loopback Detect CR11003 */

typedef enum {
    LOOP_DETECT_NO_LOOP, 	 // No loop has been detected
    LOOP_DETECT_PORT_0,         // Port 0 is known to be in a loop, don't know other port yet
    LOOP_DETECT_PORT_1,  	// Port 1 is known to be in a loop, don't know other port yet
    LOOP_DETECT_PORT_2,         // Port 2 is known to be in a loop, don't know other port yet
    LOOP_DETECT_PORT_3,         // Port 3 is known to be in a loop, don't know other port yet
    LOOP_DETECT_PORT_0_1,       // The rest of these indicate which 2 ports are in the loop
    LOOP_DETECT_PORT_0_2,
    LOOP_DETECT_PORT_0_3,
    LOOP_DETECT_PORT_1_2,
    LOOP_DETECT_PORT_1_3,
    LOOP_DETECT_PORT_2_3,
} loop_detect_t;

loop_detect_t loop_detect_state=LOOP_DETECT_NO_LOOP;;

/* The following defines are used to see which bit in the ATU destination port vector is set */
#define PORT5_DPV_MASK	0x0200
#define PORT4_DPV_MASK	0x0100
#define PORT3_DPV_MASK	0x0080
#define PORT2_DPV_MASK	0x0040
#define PORT1_DPV_MASK	0x0020
#define PORT0_DPV_MASK	0x0010

u8 spt_mac[6] = {0x01,0x80,0xc2,0x00,0x00,0x00};
#define MAX_BUSY	100					 // Max reads to wait for clear of the ATU Operation register busy bit before giving up
#define LOOP_EXPIRATION_THRESHOLD	10   // Number of seconds that must elapse with no looped packets before re-enabling port
struct timer_list loop_detect_timer;     // Linux kernel timer used to detect when a loop is removed.

// Forward Declarations
static INLINE int atu_lookup_mac(u8*, u16*);

/************** End of CR11003 Section ****************/
#endif  // CONFIG_WSTL_LOOP_DETECT

#define MOCA_WAN_OUT_OF_RESET 1
#define MOCA_LAN_OUT_OF_RESET 2
static u16 Moca_Reset_State = 0;
...
extern int ifx_ssc_txrx(char * tx_buf, u32 tx_len, char * rx_buf, u32 rx_len, u16 PLD_mask);
extern int ifx_ssc_tx(char * tx_buf, u32 tx_len, u16 PLD_mask);
extern int ifx_ssc_rx(char * tx_buf, u32 tx_len, char * rx_buf, u32 rx_len, u16 PLD_mask); 
...
static int westell_spi_mdio_read(u32 mdio_register, u16 PLD_mask)
{
    u16 mdio_port = PLD_mask;
    volatile u32 WriteData = mdio_register;
//    volatile u32 WritePreamble = 0xffffffff;
    volatile int ReadData;
//    volatile int data = IFX_SSC_MODE_TX;
//    volatile int data1 = IFX_SSC_MODE_RXTX;
    int ret = 0;
 //   if ((ret=ifx_ssc_ioctl((struct inode *)0, NULL,WESTELL_SSC_RXTX_MODE_SET, (unsigned long)&data))){
 //         printk("set TX mode failed\n");
 //         ret = -1;
 //   }
 //   else if ( (ret=ifx_ssc_tx((char *)&WritePreamble,4)) < 0){
 //        printk("write preamble failed \n");
 //   }
 //   else 
//        if ((ret=ifx_ssc_ioctl((struct inode *)0, NULL,WESTELL_SSC_RXTX_MODE_SET, (unsigned long)&data1)))
//    {
//         printk("failed to set RXTX MODE \n");
//    }
//    else 
//    { 
         if ( (ret=ifx_ssc_rx((char *)&WriteData,4,(char *)&ReadData,4,mdio_port)) < 0){
...
static int westell_spi_mdio_write(u32 mdio_register, u16 PLD_mask)
{
    int ret = 0;
    u16 mdio_port = PLD_mask;
 //   volatile int data1 = IFX_SSC_MODE_TX;
 //   volatile u32 WriteData = mdio_register;
//    volatile u32 WriteData = 0xffffffff;
 //   if ((ret=ifx_ssc_ioctl((struct inode *)0, NULL,WESTELL_SSC_RXTX_MODE_SET, (unsigned long)&data1))){
 //        printk("set TX mode failed\n");
 //        ret = -1;
 //   }
 //   else
 //   {        
 //       if ( (ret=ifx_ssc_tx((char *)&WriteData,4)) < 0){
 //            printk("write data preamble failed \n");
 //       }
 //       else
 //       {        
            u32 volatile WriteData = mdio_register;
            if ( (ret=ifx_ssc_tx((char *)&WriteData,4,mdio_port)) < 0){
...
static int westell_spi_mdio_reset(int port, int flag)
{
    int data1 = IFX_SSC_MODE_TX;
    int ret = 0;
    u16 PLD_mask = 1;
#ifdef DUAL_SPI_MDIO
    u16 reset_device;
    u16 moca_reset_value;
    if (port)
    {
        reset_device = MOCA_WAN_OUT_OF_RESET;
        PLD_mask = 2;
    }
    else
    {
        reset_device = MOCA_LAN_OUT_OF_RESET;
    }
#endif
    if (flag) {
//       printk("putting entropic in reset \n");
#ifdef DUAL_SPI_MDIO
        if (Moca_Reset_State & reset_device) 
        {
            Moca_Reset_State &= ~reset_device;
            if (Moca_Reset_State == 0) 
            {
                if (ifx_ssc_close((struct inode *)0, NULL))
                {
                    printk("spi_close fails\n");
                }
            }
            moca_reset_value = 0xc | Moca_Reset_State;
            *MARVEL_SWITCH_RESET = moca_reset_value;
        }
#else
        if (ifx_ssc_close((struct inode *)0,NULL)) 
        {
            printk("spi_close_fails\n");
        }
        *MARVEL_SWITCH_RESET = 0xd;
#endif
        
    }
    else
    { 
//        printk("taking entropic out of reset \n");
#ifdef DUAL_SPI_MDIO
        if (!(Moca_Reset_State & reset_device)) 
        {
            Moca_Reset_State |= reset_device;
            ...
            *MARVEL_SWITCH_RESET = moca_reset_value;
#else
            *MARVEL_SWITCH_RESET = 0xf;
#endif
#ifdef DUAL_SPI_MDIO
            if (Moca_Reset_State != (MOCA_WAN_OUT_OF_RESET | MOCA_LAN_OUT_OF_RESET)) 
            {
#endif
                if ((ret=ifx_ssc_open((struct inode *)0, NULL)))
                {
                    printk("spi_open failed\n");
                }
#ifdef DUAL_SPI_MDIO
            }
#endif
            ...
            if (westell_spi_mdio_write(0xffffffff,PLD_mask) == -1 )
            {
                printk("spi initial write failed \n");
            }
#ifdef DUAL_SPI_MDIO
        }
#endif
...
/*WESTELL CHANGE*/
static void westell_get_eth_stats ( struct net_device *dev, struct wstl_user_net_device_stats_t *w_device_stats, int port)
{
    //printk("PPA stat read \n");
        
    eth_get_stats(dev);
    w_device_stats->rx_packets = eth_dev[port].stats.rx_packets;
    w_device_stats->rx_bytes = eth_dev[port].stats.rx_bytes;
    w_device_stats->rx_errors = eth_dev[port].stats.rx_errors - eth_dev[port].enets_igerr;
    w_device_stats->rx_dropped = eth_dev[port].stats.rx_dropped - eth_dev[port].enets_igdrop;

    w_device_stats->tx_packets = eth_dev[port].stats.tx_packets;
    w_device_stats->tx_bytes = eth_dev[port].stats.tx_bytes;
    w_device_stats->tx_errors = eth_dev[port].stats.tx_errors - eth_dev[port].enetf_egcol;
    w_device_stats->tx_dropped = eth_dev[port].stats.tx_dropped - eth_dev[port].enetf_egdrop;


        //eth_dev[0].stats.rx_packets = eth_dev[0].rx_packets + LAN_RX_MIB_TBL->lrx_fast_tcp_pkts + LAN_RX_MIB_TBL->lrx_fast_udp_pkts;
        //BRIDGING_ETH_MIB_TBL(port)->ig_fast_pkts;
        /*For the LAN*/
    if (port == 0) {
        if (g_fwcode == FWCODE_ROUTING_ACC_D2) {
            w_device_stats->rx_packets_fastpath = LAN_RX_MIB_TBL->lrx_fast_tcp_pkts + LAN_RX_MIB_TBL->lrx_fast_udp_pkts;
        }
        else if (g_fwcode == FWCODE_BRIDGING_ACC_D3) {
            w_device_stats->rx_packets_fastpath = BRIDGING_ETH_MIB_TBL(port)->ig_fast_pkts;
        }
    }
        //eth_dev[1].stats.rx_packets = eth_dev[1].rx_packets + WAN_RX_MIB_TBL->wrx_fast_uc_tcp_pkts + WAN_RX_MIB_TBL->wrx_fast_uc_udp_pkts + WAN_RX_MIB_TBL->wrx_fast_mc_tcp_pkts + WAN_RX_MIB_TBL->wrx_fast_mc_udp_pkts;
    else if (port == 1) {
        if (g_fwcode == FWCODE_ROUTING_ACC_D2) {
            w_device_stats->rx_packets_fastpath = WAN_RX_MIB_TBL->wrx_fast_uc_tcp_pkts + WAN_RX_MIB_TBL->wrx_fast_uc_udp_pkts + WAN_RX_MIB_TBL->wrx_fast_mc_tcp_pkts + WAN_RX_MIB_TBL->wrx_fast_mc_udp_pkts;
        }
        else if (g_fwcode == FWCODE_BRIDGING_ACC_D3) {
            w_device_stats->rx_packets_fastpath = BRIDGING_ETH_MIB_TBL(port)->ig_fast_pkts;
        }
...
static int eth_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    case WESTELL_SPI_MDIO_READ:
        {
            u32 mdio_w_data;
            u32 mdio_r_data;
            u16 PLD_mask = 1;
            struct mii_ioctl_data *mdio_p;
            mdio_p = &ifr->ifr_data;
            mdio_w_data = 0x60000000 | ((mdio_p->phy_id << 23) | (mdio_p->reg_num << 18));
            if (port) 
            {
                PLD_mask = 2;
            }
            if ((mdio_r_data = westell_spi_mdio_read(mdio_w_data,PLD_mask)) == -1) 
            {
                return -EFAULT;
            }
            mdio_p->val_out = (u16)(mdio_r_data & 0xffff);
 
        }
        break;
    case WESTELL_SPI_MDIO_WRITE:
        {
            u32 mdio_w_data;
            u16 PLD_mask = 1;
            struct mii_ioctl_data *mdio_p;
            mdio_p = &ifr->ifr_data;
            mdio_w_data = 0x50020000 | ((mdio_p->phy_id << 23)|  (mdio_p->reg_num << 18) | mdio_p->val_in);
            if (port) 
            {
                PLD_mask = 2;
            }
            if (westell_spi_mdio_write(mdio_w_data,PLD_mask) == -1) 
            {
                return -EFAULT;
            }
        ...
        break;
    case WESTELL_SPI_MDIO_RESET:
        {
            int flag = ifr->ifr_ifru.ifru_ivalue;

            if (westell_spi_mdio_reset(port,flag))
            {
                return -EFAULT;
            }
        ...
        break;
/* XXX workaround to support two switches
 * 1st switch (eth0, port==0) uses address range 0x00-0x0f 
 * 2nd switch (eth1, port==1) uses address range 0x10-0x1f */
#define MDIO_FIX_PHY(phy) ((phy) | (port ? 0x10 : 0))
    ...
    case SIOCGMIIREG:
        mdio->val_out = mdio_read(MDIO_FIX_PHY(mdio->phy_id), mdio->reg_num);
        break;
  
#ifdef CONFIG_WSTL_DEV_STATS
    case WESTELL_ETH_STATS:
        {
           // printk("PPA: inside WESTELL_ETH_STATs \n");
            struct wstl_user_net_device_stats_t westell_stats;
            westell_get_eth_stats ( dev, &westell_stats, port);           
            copy_to_user( ifr->ifr_data, &westell_stats, sizeof(struct wstl_user_net_device_stats_t));          
        }
    break;
#endif
    case SIOCSMIIREG:
        mdio_write(MDIO_FIX_PHY(mdio->phy_id), mdio->reg_num, mdio->val_in);
	break;
    default:
        return -EOPNOTSUPP;
    }
...
#ifdef CONFIG_WSTL_LOOP_DETECT
/* Westell WSTL CR11003 */
/*                                                              
    This function prints out the state of the ports for debugging.    							    
*/
static INLINE void print_port_states(void) {
    u16 port_control_reg;
    port_control_reg = mdio_read(0x08,0x04);
    printk("Port 0 control register = 0x%x\n",port_control_reg);
    port_control_reg = mdio_read(0x09,0x04);
    printk("Port 1 control register = 0x%x\n",port_control_reg);
    port_control_reg = mdio_read(0x0A,0x04);
    printk("Port 2 control register = 0x%x\n",port_control_reg);
    port_control_reg = mdio_read(0x0B,0x04);
    printk("Port 3 control register = 0x%x\n",port_control_reg);
}
...
/* Westell WSTL CR11003 */
/*
    This function puts the looping ports into FORWARDING state. Used when a loop is removed. 
    The other bits in the port control register are left unchanged.
*/
static INLINE void port_forwarding_state(unsigned int port) {
    // The passed port must be one of: 0,1,2,3 (these are the only ports on which loops are detected here)
	u16 port_control_reg,modified_port_control_reg,vlan_map_reg;
	u16 phy;
	switch (port) {
	case 0:
		phy = 0x08;
	...
	port_control_reg = mdio_read(phy, 0x04);
	// Set it so bits 0-1 are 11 binary, and bits 2-15 are not changed. 
	modified_port_control_reg = port_control_reg  | 0x0003;
	// printk("port_forwarding_state for port %d 0x%x --> 0x%x\n",port,port_control_reg,modified_port_control_reg);
	mdio_write(phy,0x04,modified_port_control_reg);


	// When the port was put into listening mode, we set the force map bit, bit 8 in its port base vlan map register.
	// We clear that bit here
	vlan_map_reg = mdio_read(phy, 0x06);
	mdio_write(phy, 0x06, vlan_map_reg & ~((u16)1<<8));
}


/* Westell WSTL CR11003 */
/*                                                              
  This function puts the specified port into BLOCKING/LISTENING state. Used when a loop is detected.
  The other bits in the port control register are left unchanged.
*/
static INLINE void port_listening_state(int port) {
	// The passed port must be one of: 0,1,2,3 (these are the only ports on which loops are detected here)
	u16 port_control_reg,modified_port_control_reg,port_base_vlan_map_reg;
	u16 phy;
	switch (port) {
	case 0:
		phy = 0x08;
	...
	port_control_reg = mdio_read(phy, 0x04);
	// Set it so bits 0-1 are 01 binary, and bits 2-15 are not changed. (Turn off bit 1, and turn on bit 0.)
	modified_port_control_reg = (port_control_reg & 0xFFFD) | 0x0001;
	// printk("port_listening_state for port %d 0x%x --> 0x%x\n",port,port_control_reg,modified_port_control_reg);
	mdio_write(phy,0x04,modified_port_control_reg);


	// IN addition to putting the port in BLOCKING/LISTENING mode,
	// we also change the port based vlan register such that the force map bit is set and we map
	// management frames to the CPU. This forces the looping STP frames to get to the CPU while the loop persits
	port_base_vlan_map_reg = mdio_read(phy, 0x06);
	mdio_write(phy,0x06, (port_base_vlan_map_reg & 0xFFE0) | 1<<5 | 1<<8);  // Turn off bits 0,1,2,3, and 4, & turn on bits 5 and 8

}


/* Westell WSTL CR11003 */
// This function adds the spanning tree multicast mac address to the ATU with dbnum=1
// The entry is marked as a STATIC MGMT entry so that looping SPT packets will tunnel through blocked ports
// The two ports involved in the loop are changed to use dbnum=1
#if 0 /* Currently not using atu_add_mgmt() or atu_remove_mgmt() but leaving them here in case we need them later. */
static INLINE void atu_add_mgmt() {
    int num_busy,check_result;
    u16 atu_op_reg;
    u16 atu_data_reg;
    u16 port_based_vlan_reg;
    u16 *p16;

    // First, wait for the busy bit to clear
    atu_op_reg = mdio_read(0x0f,0x0B); // offset 0x0B is the op register 
    num_busy=0;
    while (atu_op_reg & 0x8000) {
	num_busy++;
	if (num_busy >= MAX_BUSY) {
	    printk("CR11003: atu_add_mgmt : Error - Couldn't issue LOAD request, busy bit timeout.\n");
	    return;
	}
	atu_op_reg = mdio_read(0x0f,0x0B);
    }

    // Next, load the ATU mac registers with the STP DA  
    // Register 0x0D gets bits 47-32 of the mac, which is stored in spt_mac[0] and spt_mac[1]
    p16 = (u16*)&spt_mac[0];
    mdio_write(0x0f, 0x0D, *p16);
    // Register 0x0E gets bits 16-31 of the mac, which is stored in spt_mac[2] and spt_mac[3]
    p16 = (u16*)&spt_mac[2]; 
    mdio_write(0x0f, 0x0E, *p16);
    // Register 0x0F gets bits 0-15 of the mac, which is stored in spt_mac[4] and spt_mac[5]
    p16 = (u16*)&spt_mac[4]; 
    mdio_write(0x0f, 0x0F, *p16);

    // Next, write the ATU Data register - 0xC. Specifies the desired entry state and the destination port vector
    // We want to set the bits in the DPV for the CPU (port 5, bit 9) 
    
    atu_data_reg = 0x0206; // Use entry state 0x6 (static mgmt) and DPV has bits set for port 5 only
    mdio_write(0x0f,0x0C,atu_data_reg);
    
    // Now write the ATU Operation Regisgter (0x0B)
    atu_op_reg = 0xB001; // This sets the busy bit and defines the operation as "load or purge", and sets the dbnum field to 1
    mdio_write(0x0f, 0x0B, atu_op_reg);


    // Now, we have to set the loopy ports to use dbnum=1
    // To do this, we set bit 12 of port register 6 for each of the 2 ports
    switch (loop_detect_state) {
    case LOOP_DETECT_PORT_0_1:
	port_based_vlan_reg = mdio_read(0x08,0x06); 
	mdio_write(0x08, 0x06, port_based_vlan_reg | (1 << 12));
	port_based_vlan_reg = mdio_read(0x09,0x06); 
	mdio_write(0x09, 0x06, port_based_vlan_reg | (1 << 12));
	break;
    case LOOP_DETECT_PORT_0_2:
	port_based_vlan_reg = mdio_read(0x08,0x06); 
	mdio_write(0x08, 0x06, port_based_vlan_reg | (1 << 12));
	port_based_vlan_reg = mdio_read(0x0A,0x06); 
	mdio_write(0x0A, 0x06, port_based_vlan_reg | (1 << 12));
	break;
    case LOOP_DETECT_PORT_0_3:
	port_based_vlan_reg = mdio_read(0x08,0x06); 
	mdio_write(0x08, 0x06, port_based_vlan_reg | (1 << 12));
	port_based_vlan_reg = mdio_read(0x0B,0x06); 
	mdio_write(0x0B, 0x06, port_based_vlan_reg | (1 << 12));
	break;
    case LOOP_DETECT_PORT_1_2:
	port_based_vlan_reg = mdio_read(0x09,0x06); 
	mdio_write(0x09, 0x06, port_based_vlan_reg | (1 << 12));
	port_based_vlan_reg = mdio_read(0x0A,0x06); 
	mdio_write(0x0A, 0x06, port_based_vlan_reg | (1 << 12));
	break;
    case LOOP_DETECT_PORT_1_3:
	port_based_vlan_reg = mdio_read(0x09,0x06); 
	mdio_write(0x09, 0x06, port_based_vlan_reg | (1 << 12));
	port_based_vlan_reg = mdio_read(0x0B,0x06); 
	mdio_write(0x0B, 0x06, port_based_vlan_reg | (1 << 12));
	break;
    case LOOP_DETECT_PORT_2_3:
	port_based_vlan_reg = mdio_read(0x0A,0x06); 
	mdio_write(0x0A, 0x06, port_based_vlan_reg | (1 << 12));
	port_based_vlan_reg = mdio_read(0x0B,0x06); 
	mdio_write(0x0B, 0x06, port_based_vlan_reg | (1 << 12));
...
/* Westell WSTL CR11003 */
// Remove the MGMT entry from the ATU that was added when a loop was detected.
static INLINE void atu_remove_mgmt(void) {
    int num_busy;
    u16 atu_op_reg;
    u16 atu_data_reg;
    u16 port_based_vlan_reg;
    ...
    printk("CR11003: Entered atu_remove_mgmt()...\n");
	...
	num_busy++;
	if (num_busy >= MAX_BUSY) {
	    // printk("CR11003: atu_remove_mgmt : Error - Couldn't issue LOAD request, busy bit timeout.\n");
	    return;
	}
	atu_op_reg = mdio_read(0x0f,0x0B);
    }
    ...
    // Next, write the ATU Data register - 0xC. Specifies entry state of 0 to indicate purge 
    atu_data_reg = 0x0000; 
    mdio_write(0x0f,0x0C,atu_data_reg);
    
    // Now write the ATU Operation Regisgter (0x0B)
    atu_op_reg = 0xB001; // This sets the busy bit and defines the operation as "load or purge" (this one will be a purge since entry state set to 0 above). Also set the dbnum to 1
    mdio_write(0x0f, 0x0B, atu_op_reg);



    // Now restore the ports to use database number 0 again.
    switch (loop_detect_state) {
    case LOOP_DETECT_PORT_0_1:
	port_based_vlan_reg = mdio_read(0x08,0x06); 
	mdio_write(0x08, 0x06, port_based_vlan_reg & ~(1 << 12));
...
/* Westell WSTL CR11003 */
// Determines the contents of the ATU data register for the specified MAC
// The ATU data register contents are written to pResult and 0 is returned if success
// If the MAC didn't have an entry in the ATU, nonzero is returned and nothing is written in pResult
static INLINE int atu_lookup_mac(u8* pMac, u16* pResult) {

	int lcv,num_busy;
	u8 lbmac[6];
	u16 atu_op_reg,atu_data_reg,maclow,macmid,machi;
	u16 *p16;

	// Try to get the ATU entry for the passed in mac address 
	memcpy(lbmac, pMac, sizeof(lbmac));

	// Subract 1 from the mac address that we want to look up, that's how the ATU GET_NEXT operation works.
	for (lcv = 5; lcv >= 0; lcv--)
	{
		lbmac[lcv]--;
		if (lbmac[lcv] != 0xFF)
	...
	while (atu_op_reg & 0x8000) {
		num_busy++;
		if (num_busy >= MAX_BUSY) {
			// printk("CR11003 - Error - Couldn't issue GET_NEXT request, busy bit timeout.\n");
			return 1;
		}
		atu_op_reg = mdio_read(0x0f,0x0B);
	}

	// Next, load the ATU mac registers with (mac-1)
	// Register 0x0D gets bits 47-32 of the mac, which is stored in lbmac[0] and lbmac[1]
	p16 = (u16*)&lbmac[0];
	mdio_write(0x0f, 0x0D, *p16);
	// Register 0x0E gets bits 16-31 of the mac, which is stored in lbmac[2] and lbmac[3]
	p16 = (u16*)&lbmac[2]; 
	mdio_write(0x0f, 0x0E, *p16);
	// Register 0x0F gets bits 0-15 of the mac, which is stored in lbmac[4] and lbmac[5]
	p16 = (u16*)&lbmac[4]; 
	mdio_write(0x0f, 0x0F, *p16);

	// Now write the ATU Operation Regisgter (0x0B)
	atu_op_reg = 0xC000; // This sets the busy bit and defines the operation as "get next"
	mdio_write(0x0f, 0x0B, atu_op_reg);

	// We've made the GET_NEXT request, now we wait for busy bit to clear so we can read the results
	atu_op_reg = mdio_read(0x0f,0x0B); // offset 0x0B is the op register 
	num_busy=0;
	while (atu_op_reg & 0x8000) {
		num_busy++;
		if (num_busy >= MAX_BUSY) {
			// printk("CR11003 - Error - Couldn't get GET_NEXT results, busy bit timeout.\n");
			return 2;
		}
		atu_op_reg = mdio_read(0x0f,0x0B);
	}
	atu_data_reg = mdio_read(0x0f,0x0C);
	maclow = mdio_read(0x0f,0x0f);
	macmid = mdio_read(0x0f,0x0e);
	machi  = mdio_read(0x0f,0x0d);
	// printk("CR11003 - For mac %x %x %x atu_data_reg = 0x%x\n",machi,macmid,maclow,atu_data_reg);

	// In order for the result to be valid, the mac we read back must equal the mac we wanted

	p16 = (u16*)(pMac);
	if (machi != *p16) {
		// printk("CR11003 - machi doesn't match,  atu_lookup_mac returning failure.\n");
		return 3;
	}
	p16 = (u16*)(pMac+2);
	if (macmid != *p16) {
		// printk("CR11003 - macmid doesn't match,  atu_lookup_mac returning failure.\n");
		return 4;
	}
	p16=(u16*)(pMac+4);
	if (maclow != *p16) {
		// printk("CR11003 - maclow doesn't match,  atu_lookup_mac returning failure.\n");
		return 4;
	}

	// The mac returned matches the one queried.
	// printk("atu_lookup_mac returning success.\n");
	*pResult = atu_data_reg;
	return 0;

}

/* Westell WSTL CR11003 */
/* This is a callback function called when the loop detect timer expires. */
void loop_detect_timer_expired(unsigned long arg) {
    // printk("Loop timer expired.\n");
    // Currently not using the argument...

    // If the mgmt entry is in the atu, we can't detect new loops, so we have to remove
    // it each time after a loop is removed. This is because with that ATU entry,
    // the switch doesn't "learn" from looping STP packets so we can't identify the offending port.
//    atu_remove_mgmt(); // Loop gone, remove the ATU entry that we added when the loop was detected.

    // Put the ports back to "forwarding" mode
    // We don't know which of the 2 we put in listening mode when the loop was detected (only one was done)
    // So we assure here that both of them are in forwarding state
    switch (loop_detect_state) {
    case LOOP_DETECT_PORT_0:
	printk("Loop no longer detected. Restoring port 0 to forwarding state.\n");
	port_forwarding_state(0);
	break;
    case LOOP_DETECT_PORT_1:
	printk("Loop no longer detected. Restoring port 1 to forwarding state.\n");
	port_forwarding_state(1);
	break;
    case LOOP_DETECT_PORT_2:
	printk("Loop no longer detected. Restoring port 2 to forwarding state.\n");
	port_forwarding_state(2);
	break;
    case LOOP_DETECT_PORT_3:
	printk("Loop no longer detected. Restoring port 3 to forwarding state.\n");
	port_forwarding_state(3);
	break;
    default:
	break;
    }
    // Set the state variable to indicate no loop currently detected, all ports forwarding.
    loop_detect_state=LOOP_DETECT_NO_LOOP;
}

/* Westell WSTL CR11003 */
/* This is a function that is called when a loop is detected */
void loop_detected(int port) {
    	if ((port < 0) || (port>3)) {
	    printk("Loop detect error - loop_detected() called with invalid port number %d\n",port);
	    return;
	}
	 printk("Switch loop detected on PORT %d current state %d.\n",port, loop_detect_state);
	switch (loop_detect_state) {
	case LOOP_DETECT_NO_LOOP:
	    // No previous loop
	    port_listening_state(port);
	    init_timer(&loop_detect_timer);
	    loop_detect_timer.function = loop_detect_timer_expired;
	    loop_detect_timer.expires = jiffies + LOOP_EXPIRATION_THRESHOLD*HZ;
	    add_timer(&loop_detect_timer);
	    // atu_add_mgmt();
	    if (port==0) {
		loop_detect_state = LOOP_DETECT_PORT_0;
	    } else if (port==1) {
		loop_detect_state = LOOP_DETECT_PORT_1;
	    } else if (port==2) {
		loop_detect_state = LOOP_DETECT_PORT_2;
	    } else if (port==3) {
		loop_detect_state = LOOP_DETECT_PORT_3;
	    }
	    break;
	default:
	    break;
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
...
#ifdef CONFIG_WSTL_LOOP_DETECT
    // Local variables for Loop detect WSTL Westell CR11003 
    int port;
	u16 atu_data_reg;
	int lookup_result;
#endif  // CONFIG_WSTL_LOOP_DETECT
...
#ifdef CONFIG_WSTL_LOOP_DETECT
    // Westell WSTL CR 11003 section
    port = get_port(ndev);
	// End CR 11003 section
#endif    // CONFIG_WSTL_LOOP_DETECT
  ...
  #else	
    #ifdef CONFIG_WSTL_LOOP_DETECT
		/* Test for loopback detection CR # 11003 */
		// Only check for loopback on the lan switch not wan (indicated by port==0)
		if (port==0) {	// If it's LAN
		    if (memcmp(skb->mac.ethernet->h_source, skb->dev->dev_addr, ETH_ALEN) == 0) { // If it's a looped packet
			switch (loop_detect_state) {
			case LOOP_DETECT_NO_LOOP:
			    // Lookup the cpu's mac in the ATU to see what port it moved to
			    lookup_result = atu_lookup_mac(skb->dev->dev_addr,&atu_data_reg);
			    if (lookup_result==0) {
				if (atu_data_reg & PORT0_DPV_MASK ) {
				    loop_detected(0);
				} else if (atu_data_reg & PORT1_DPV_MASK) {
				    loop_detected(1);
				} else if (atu_data_reg & PORT2_DPV_MASK) {
				    loop_detected(2);
				} else if (atu_data_reg & PORT3_DPV_MASK) {
				    loop_detected(3);
				} else {
				    // printk("Looped packet rxvd but couldn't identify offending port\n");
				}
			    } 
			    break;
			default:
			    // There was already a loop detected and the port is known. Just kick the timer.
			    // printk("timer kick.\n");
			    mod_timer(&loop_detect_timer,jiffies + LOOP_EXPIRATION_THRESHOLD*HZ);
			    // Drop this packet so it doesn't go to the learning bridge
			    edev->rx_dropped++;
                dev_kfree_skb_any(skb); // CR15530 VZ Issue 371 - Router becomes inoperable when a loop is created in an external switch connected to it.
			    return 0; 
			    break;
...
#ifdef CONFIG_WESTELL
    *(unsigned long *)0xBE100B1C =  0x00007f8e;//(*(unsigned long *)0xBE100B1C & 0x0000b859) | 0x00004786;
    *(unsigned long *)0xBE100B20 =  0x000047a6;//(*(unsigned long *)0xBE100B20 & 0x0000b859) | 0x000047a6;
    *(unsigned long *)0xBE100B4C =  0x000063af;//(*(unsigned long *)0xBE100B4C & 0x00008ff7) | 0x00006008;
	*(unsigned long *)0xBE100B50 =  0x00007008;//(*(unsigned long *)0xBE100B50 & 0x00008ff7) | 0x00007008;

	if ( mii1_mode == MII_MODE )
	{
	    *(unsigned long *)0xBE100B18 = (*(unsigned long *)0xBE100B18 & 0x0000b859) | 0x00004020;
		*(unsigned long *)0xBE100B48 = (*(unsigned long *)0xBE100B48 & 0x00008ff7) | 0x00007000;
		*(unsigned long *)0xBE100B24 = *(unsigned long *)0xBE100B24 | 0x00004020;
		*(unsigned long *)0xBE100B54 = *(unsigned long *)0xBE100B54 | 0x00007000;
		printk("MII1 = MII_MODE");
	}
	else
	{
	    *(unsigned long *)0xBE100B18 =  0x0000b79f;//(*(unsigned long *)0xBE100B18 & 0x0000b859) | 0x00000786;
		*(unsigned long *)0xBE100B48 =  0x00000bee;//(*(unsigned long *)0xBE100B48 & 0x00008ff7) | 0x00000008;
		*(unsigned long *)0xBE100B24 =  0x0000ffff;//*(unsigned long *)0xBE100B24 | 0x00000786;
		*(unsigned long *)0xBE100B54 =  0x0000ffff;//*(unsigned long *)0xBE100B54 | 0x00000008;
		printk("MII1 = REV_MII_MODE\n");
	}    
#else
...
static INLINE void init_etop(int fwcode, int mii0_mode, int mii1_mode)
{
...
#ifdef CONFIG_WESTELL
    // set turbo mode for mii0
    *ETOP_CFG |= 1 << 2;
    // set turbo mode for mii1
    *ETOP_CFG |= 1 << 5;
#endif
    ...
    *ETOP_MDIO_CFG    = 0x00;
    ...
    {
        switches_initialized = 1;   /* only initialize switches once ! */
        /*
         * Put Phy ports in powerdown state
         * and enable pause negotiation for ports 0 to 3
         */
        for (i = 0; i < 4; i++)
        {
           mdio = mdio_read(i, 0);
           mdio_write(i, 0, mdio | (1 << 11));
           if (i < 4) 
           {
               mdio = mdio_read(i,4);
               mdio_write(i,4,mdio | (1 << 10));
           }
        }
        ...
        //putting used ports in fowarding state 
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x08 << 21) | (0x04 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x08 << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));

        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x09 << 21) | (0x04 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x09 << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));

        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x0a << 21) | (0x04 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x0a << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));

        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x0b << 21) | (0x04 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x0b << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));

//        while(*ETOP_MDIO_ACC&0x80000000);
//        *ETOP_MDIO_ACC = (0xC0000000 | (0x0c << 21) | (0x04 << 16));
//        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
//        *ETOP_MDIO_ACC = (0x80000000 | (0x0c << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));
// Set moca Pause enable
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x0c << 21) | (0x01 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x0c << 21) | (0x01 << 16) | ((mdio & 0xffff) | 0xc0));
// Set cpu Pause enable
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x0d << 21) | (0x01 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x0d << 21) | (0x01 << 16) | ((mdio & 0xffff) | 0xc0));

        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x0d << 21) | (0x04 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x0d << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));

        // WAN switch
        //Enable Port 0
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x18 << 21) | (0x04 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x18 << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3)); 

//       while(*ETOP_MDIO_ACC&0x80000000);
//        *ETOP_MDIO_ACC = (0xC0000000 | (0x19 << 21) | (0x04 << 16));
//        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
//        *ETOP_MDIO_ACC = (0x80000000 | (0x19 << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));

//        while(*ETOP_MDIO_ACC&0x80000000);
//        *ETOP_MDIO_ACC = (0xC0000000 | (0x1a << 21) | (0x04 << 16));
//        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
//        *ETOP_MDIO_ACC = (0x80000000 | (0x1a << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));

//        while(*ETOP_MDIO_ACC&0x80000000);
//        *ETOP_MDIO_ACC = (0xC0000000 | (0x1b << 21) | (0x04 << 16));
//        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
//        *ETOP_MDIO_ACC = (0x80000000 | (0x1b << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x3));
// 

        //Enable Port 4 (CPU)
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x1c << 21) | (0x04 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x1c << 21) | (0x04 << 16) | ((mdio & 0xffff) | 0x03));  
       // Set cpu Pause enable pause enable
        //Port 4
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x1c << 21) | (0x01 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x1c << 21) | (0x01 << 16) | ((mdio & 0xffff) | 0xc0));
...
// Set moca Pause enable
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x1d << 21) | (0x01 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x1d << 21) | (0x01 << 16) | ((mdio & 0xffff) | 0xc0));
#endif

/*Let the management switch driver bring it up*/
#if 0
        while(*ETOP_MDIO_ACC&0x80000000);
        *ETOP_MDIO_ACC = (0xC0000000 | (0x1d << 21) | (0x01 << 16));
        while((mdio = *ETOP_MDIO_ACC)&0x80000000);
        *ETOP_MDIO_ACC = (0x80000000 | (0x1d << 21) | (0x01 << 16) | ((mdio & 0xffff) | 0x03));
#endif
    }
#endif // USE_WESTELL_BOARD


    *ENET_MAC_CFG(0)  = 0x080f;  //set pause enable 
    ...
    *ENET_MAC_CFG(1)  = 0x080f;  // set pause enable

    ppe_set_enable_pause(-1);    // (-1=use last setting) cr15452 enable/disable flow control on vdsl2 in bridge mode.
    ...
    *(volatile u32 *)0xBE104144 = (*(volatile u32 *)0xBE104144 & ~0x7C) | 0x28;
...
#if defined(CONFIG_WESTELL)
static INLINE void board_init(void)
{
}
#elif defined(ENABLE_DANUBE_BOARD) && ENABLE_DANUBE_BOARD
static INLINE void board_init(void)
{
}
...
static INLINE void proc_file_create(void)
{
    ...
    res = create_proc_read_entry("setren",
                                  0,
                                  g_eth_proc_dir,
                                  NULL,
                                  NULL);
    if ( res )
        res->write_proc = proc_write_stren;
    ...
    res = create_proc_read_entry("spi",
                                  0,
                                  g_eth_proc_dir,
                                  proc_read_spi,
                                  NULL);
    if ( res )
        res->write_proc = proc_write_spi;

    res = create_proc_read_entry("mdio",
                                  0,
                                  g_eth_proc_dir,
                                  NULL,
                                  NULL);
    if ( res )
        res->write_proc = proc_write_mdio;
...
static int proc_read_spi(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    int ret;
    struct ifx_ssc_hwopts hwopts;

    MOD_INC_USE_COUNT;

    if ((ret=ifx_ssc_ioctl((struct inode *)0, NULL,WESTELL_SSC_HWOPTS_GET, (unsigned long)&hwopts))){
        printk("set RXTX mode fails\n");
    }
    else
    {
        len += sprintf(page + off + len, "AbortErrDetect = %08X)\n", hwopts.AbortErrDetect);
        len += sprintf(page + off + len, "rxOvErrDetect = %08X)\n", hwopts.rxOvErrDetect);
        len += sprintf(page + off + len, "rxUndErrDetect = %08X)\n", hwopts.rxUndErrDetect);
        len += sprintf(page + off + len, "txUndErrDetect = %08X)\n", hwopts.txUndErrDetect);
        len += sprintf(page + off + len, "txOvErrDetect = %08X)\n", hwopts.txOvErrDetect);
        len += sprintf(page + off + len, "echoMode = %08X)\n", hwopts.echoMode);
        len += sprintf(page + off + len, "loopBack = %08X)\n", hwopts.loopBack);
        len += sprintf(page + off + len, "idleValue = %08X)\n", hwopts.idleValue);
        len += sprintf(page + off + len, "clockPolarity = %08X)\n", hwopts.clockPolarity);
        len += sprintf(page + off + len, "headingControl = %08X)\n", hwopts.headingControl);
        len += sprintf(page + off + len, "clockPhase = %08X)\n", hwopts.clockPhase);
        len += sprintf(page + off + len, "headingControl = %08X)\n", hwopts.headingControl);
        len += sprintf(page + off + len, "dataWidth = %08X)\n", hwopts.dataWidth);
        len += sprintf(page + off + len, "masterSelect = %08X)\n", hwopts.masterSelect);
        len += sprintf(page + off + len, "modeRxTx = %08X)\n", hwopts.modeRxTx);
        len += sprintf(page + off + len, "gpoCs = %08X)\n", hwopts.gpoCs);
        len += sprintf(page + off + len, "gpoInv = %08X)\n", hwopts.gpoInv);
    }
...
static int proc_write_spi(struct file *file, const char *buf, unsigned long count, void *data)
{
	int ret=0;
	u32 WriteData;
    u32 data1;
    ...
    {
        if ( stricmp(p1, "wl") == 0 || stricmp(p1, "ww") == 0 || stricmp(p1, "o") == 0 || stricmp(p1, "c") == 0 || stricmp(p1, "s") == 0 || stricmp(p1, "m") == 0 || stricmp(p1, "rl") == 0 || stricmp(p1, "rw") == 0)
            break;
     ...
     if ( p1[0] == 'w' )
     {
        ...
        {        
            ignore_space(&p2, &len);
            WriteData = get_number(&p2, &len, 1);
            printk("proc_write_spi: writing data %x \n",WriteData);

            if ( (ret=ifx_ssc_tx((char *)&WriteData,4,PLD_mask)) < 0){
                 printk("write/read data failed \n");
            }
     ...
     else if ( p1[0] == 'r' )
     {
        ...
        data1 = (u32)IFX_SSC_MODE_RXTX;
        if ((ret=ifx_ssc_ioctl((struct inode *)0, NULL,WESTELL_SSC_RXTX_MODE_SET, (unsigned long)&data1))){
              printk("set RXTX mode failed\n");
        }
        else
        {        
            ignore_space(&p2, &len);
            WriteData = get_number(&p2, &len, 1);
            printk("proc_read_spi: writing data %x \n",WriteData);

            if ( (ret=ifx_ssc_rx((char *)&WriteData,4,(char *)&ReadData,4,PLD_mask)) < 0){
                printk("read data failed \n");
            }
            else
            {
                printk("spi data read %x \n",ReadData);
     ...
     {	
         ...
         {             
         printk("spi wl = write lan | ww = write lan | o = open |  c = close\n");
         printk("         read lan | rw = read lan");
     }
...
static int proc_write_stren(struct file *file, const char *buf, unsigned long count, void *data)
{
   ...
   while ( get_token(&p1, &p2, &len, &colon) )
    {
        if ( stricmp(p1, "c") == 0 || stricmp(p1, "s") == 0)
            break;
    ...
    if ( *p1 == 'c' )
    {
        *DANUBE_ASC1_WHBSTATE = DANUBE_ASC1_WHBSTATE_CLRREN;
    }
    else if ( *p1 == 's' )
    {
        *DANUBE_ASC1_WHBSTATE = DANUBE_ASC1_WHBSTATE_SETREN;
    }
...
#endif
#define TWINPASS_PPE_ETOP_MDIO_ACC                   ((volatile u32*)(0xBE191804))

static int proc_write_mdio(struct file *file, const char *buf, unsigned long count, void *data)
{
    char *p1, *p2;
    int len;
    int colon;
   ...
   {
       ignore_space(&p2, &len);
       addr = (int)get_number(&p2, &len, 1);

       ignore_space(&p2, &len);
       reg = (int)get_number(&p2, &len, 1);

       ignore_space(&p2, &len);
       value = (unsigned short)get_number(&p2, &len, 1);

       printk("write Addr %02x Reg %02x Value %02X\n", addr, reg, value);
       while(*TWINPASS_PPE_ETOP_MDIO_ACC&0x80000000);
       *TWINPASS_PPE_ETOP_MDIO_ACC = (0x80000000 | ((addr & 0x1f) << 21) | ((reg & 0x1f) << 16) | (value & 0xffff));
   }
   else if ( *p1 == 'r' )
   {
       ignore_space(&p2, &len);
       addr = (unsigned long *)get_number(&p2, &len, 1);

       ignore_space(&p2, &len);
       reg = (unsigned long *)get_number(&p2, &len, 1);

       printk("read Addr %02x Reg %02x ", addr, reg);
       while(*TWINPASS_PPE_ETOP_MDIO_ACC&0x80000000);
       *TWINPASS_PPE_ETOP_MDIO_ACC = (0xC0000000 | ((addr & 0x1f) << 21) | ((reg & 0x1f) << 16));
       while((mdio = *TWINPASS_PPE_ETOP_MDIO_ACC)&0x80000000);
       printk("value = 0x%04X  %d\n", (mdio&0xffff),(mdio&0xffff));
   }
   MOD_DEC_USE_COUNT;
...
#define PPE_ENABLE_PAUSE_MASK   0x0010
#define PHY_ENABLE_PAUSE_MASK   0x00c0

// cr15194 enable/disable flow control on vdsl2
void ppe_set_enable_pause(int set_enabled)
{
    ...
    if (set_enabled == -1)
        set_enabled = last_setting; // incase ppa is re-downloaded. (CR15452 fix)

    value = *ENET_MAC_CFG(0); //    = 0x081F;
    value1 = *ENET_MAC_CFG(1); 
    ...
    *ETOP_MDIO_ACC = (0xC0000000 | (0x1d << 21) | (0x01 << 16));
    ...
    if (set_enabled)
    {
        value |= PPE_ENABLE_PAUSE_MASK; //mask pause on
        value1 |= PPE_ENABLE_PAUSE_MASK; //mask pause on
        // Set Pause enable for vdsl phy port (CR15500)
        *ETOP_MDIO_ACC = (0x80000000 | (0x1d << 21) | (0x01 << 16) | ((mdio & 0xffff) | PHY_ENABLE_PAUSE_MASK));
        //printk("PPE Enable PAUSE\n");
    }
    else
    {
        value &= ~(PPE_ENABLE_PAUSE_MASK); //mask pause off
        value1 &= ~(PPE_ENABLE_PAUSE_MASK); //mask pause off
        // Clear Pause enable for vdsl phy port (CR15500)
        *ETOP_MDIO_ACC = (0x80000000 | (0x1d << 21) | (0x01 << 16) | ((mdio & 0xffff) & ~(PHY_ENABLE_PAUSE_MASK)));
        //printk("PPE disable PAUSE\n");
    }

    // Set Pause enable for ethernet mac ctlr
    *ENET_MAC_CFG(0) = value;
    *ENET_MAC_CFG(1) = value1;

    last_setting = set_enabled;
#endif
    return;
}

int ppe_bit_mask_reg(int addr, int reg, int bit, int state_on)
{
    unsigned short value;
    ...
    if (bit<0 || bit > 16)
    {
        printk("Write PPE mask failed (bad bit)!\n");
        return 1; //error
    }
    ...
    while(*TWINPASS_PPE_ETOP_MDIO_ACC&0x80000000);
    *TWINPASS_PPE_ETOP_MDIO_ACC = (0xC0000000 | ((addr & 0x1f) << 21) | ((reg & 0x1f) << 16));
    while((value = *TWINPASS_PPE_ETOP_MDIO_ACC)&0x80000000);

    value_mask = (1 << bit);

    if (state_on)
        value |= value_mask; //mask it on
    else
        value &= ~(value_mask); //mask it off

    printk("Write PPE addr[%x] reg[%x] masked value = 0x%04X  %d\n", addr, reg, (value&0xffff),(value&0xffff));
...
int __init danube_eth_init(void)
{
    ...
    init_pmu();
    Moca_Reset_State = 0;
#ifndef DUAL_SPI_MDIO
    *MARVEL_SWITCH_RESET =  0x02;//put switch in reset take MoCA out of reset first
#else
    *MARVEL_SWITCH_RESET = 0x03;
#endif
    init_gpio(MII1_MODE_SETUP);

    udelay(1000);
    *MARVEL_SWITCH_RESET = 0x0f;
    udelay(2000);
#ifdef DUAL_SPI_MOCA
    *MARVEL_SWITCH_RESET = 0xc;  // Put Moca's in reset
#endif
    switches_initialized = 0;   /* switches were reset so need to be initialized */
    init_etop(g_fwcode, MII0_MODE_SETUP, MII1_MODE_SETUP);
