...
int aes_set_key(void *ctx_arg, const uint8_t *in_key, unsigned int key_len, uint32_t *flags)
{
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
			...
			int mode)
{
	...
	/* Test the KV flag */
	while (!encdec && !aes->controlr.KV)
	{
	}

	/* In case DAU is 0, IV is updated automatically by the HW.
	 * In that case, make sure to pass an IV on the first block and NULL on
	 * all other blocks.
	 */
	if(mode > 1 && (aes->controlr.DAU || iv_arg))
	{
		aes->IV3R = (*(u32 *) iv_arg);
	...
	{
    	case 16:
    		aes->ID3R =  ENDIAN_SWAP(*((u32 *) in_arg + 0));
    		aes->ID2R =  ENDIAN_SWAP(*((u32 *) in_arg + 1));
    		aes->ID1R =  ENDIAN_SWAP(*((u32 *) in_arg + 2));
    		aes->ID0R =  ENDIAN_SWAP(*((u32 *) in_arg + 3));		/* start crypto */
    		break;

    	case 8:
    		aes->ID1R =  ENDIAN_SWAP(*((u32 *) in_arg + 0));
    		aes->ID0R =  ENDIAN_SWAP(*((u32 *) in_arg + 1));		/* start crypto */
    		break;

    	case 4:
    		aes->ID0R =  ENDIAN_SWAP(*((u32 *) in_arg));			/* start crypto */
    		break;

    	case 2:
    		aes->ID0R =  ENDIAN_SWAP((u32) *((u16 *) in_arg));	/* start crypto */
    		break;
