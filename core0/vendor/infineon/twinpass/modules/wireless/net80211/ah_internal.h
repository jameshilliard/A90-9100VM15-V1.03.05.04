 ...
 * All rights reserved.
 *
 * $Id: ah_internal.h,v 1.1.4.1 2007/01/07 12:46:50 shmulik Exp $
 */
...
struct ath_hal_private {
	...
	HAL_BOOL	(*ah_getChannelEdges)(struct ath_hal*,u_int16_t ,
				u_int16_t *, u_int16_t *);
	...
	/* GPIO */
	HAL_BOOL	(*ah_gpioCfgOutput)(struct ath_hal *, u_int32_t);
	HAL_BOOL	(*ah_gpioCfgInput)(struct ath_hal *, u_int32_t);
	u_int32_t	(*ah_gpioGet)(struct ath_hal *, u_int32_t );
	HAL_BOOL	(*ah_gpioSet)(struct ath_hal *, u_int32_t, u_int32_t );
	...
	HAL_CHANNEL_INTERNAL ah_channels[64];	/* calculated channel list */
