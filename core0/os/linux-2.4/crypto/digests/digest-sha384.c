/* $Id: digest-sha384.c,v 1.1 2003/10/09 08:31:05 sergey Exp $
 * 
...
static void
SHA384Update(sha384_ctx_t *ctx, const u8 *input, u32 inputLen)
{
	u32 i, index, partLen;

	/* Compute number of bytes mod 128 */
	index = (u32)((ctx->count[0] >> 3) & 0x7F);
	...
	if (inputLen >= partLen) {
		memcpy(&ctx->buf[index], input, partLen);
		SHA384Transform(ctx->state, ctx->buf);

		for (i = partLen; i + 127 < inputLen; i += 128)
			SHA384Transform(ctx->state, &input[i]);
		index = 0;
	} else {
		i = 0;
	}
	...
	memcpy(&ctx->buf[index], &input[i], inputLen-i);
}

static void
SHA384Final(sha384_ctx_t *ctx, u8 *digest)
{
	...
	SHA384Update(ctx, padding, padLen);

	/* Append length (before padding) */
	SHA384Update(ctx, bits, sizeof(bits));

	/* Store state in digest */
	for (i = j = 0; i < 6; i++, j += 8) {
		u64 t2 = ctx->state[i];
		digest[j+7] = t2 & 0xff; t2>>=8;
		digest[j+6] = t2 & 0xff; t2>>=8;
		digest[j+5] = t2 & 0xff; t2>>=8;
		digest[j+4] = t2 & 0xff; t2>>=8;
...
static int
sha384_open(struct digest_context *cx, int atomic)
{
	sha384_ctx_t *const ctx = (sha384_ctx_t *) cx->digest_info;
...
static int
sha384_update(struct digest_context *cx, const u8 *in, int size, int atomic)
{
	sha384_ctx_t *const ctx = (sha384_ctx_t *) cx->digest_info;

	SHA384Update(ctx, in, size);

	return 0;
}

static int
sha384_digest(struct digest_context *cx, u8 *out, int atomic)
{
	sha384_ctx_t *const ctx = (sha384_ctx_t *) cx->digest_info;

	sha384_ctx_t *const ctx_tmp = kmalloc (sizeof *ctx_tmp, GFP_KERNEL);
...
static int
sha384_close(struct digest_context *cx, u8 *out, int atomic)
{
	sha384_ctx_t *const ctx = (sha384_ctx_t *) cx->digest_info;

	static u8 tmp[48]; /* /dev/null - buf */

	SHA384Final(ctx, out ? out : tmp);

	return 0;
}

#define DIGEST_ID		sha384
#define DIGEST_BLOCKSIZE	48
