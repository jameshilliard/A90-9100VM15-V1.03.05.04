...
static char *hex2mem(char *buf, char *mem, int count, int may_fault)
{
	...
	for (i=0; i<count; i++)
	{
		ch = hex(*buf++) << 4;
		ch |= hex(*buf++);
		if (kgdb_write_byte(ch, (unsigned *)(mem++)) != 0)
			return 0;
	}
...
void *clear_debug_trap(unsigned char tt, unsigned char signo)
{
	struct hard_trap_info *ht;

	for (ht = hard_trap_info; ht->tt && ht->signo; ht++)
	{
		if (ht->signo == signo && ht->tt == tt)
		{
			return set_except_vector(ht->tt,
			    saved_vectors[ht->tt]);
		}
	}
...
void set_debug_trap(unsigned char tt, unsigned char signo, void *trap)
{
	struct hard_trap_info *ht;
	...
	for (ht = hard_trap_info; ht->tt && ht->signo; ht++)
	{
		if (ht->signo == signo && ht->tt == tt)
		{
			saved_vectors[ht->tt] = set_except_vector(ht->tt,
			    trap);
			return;
		}
	}
