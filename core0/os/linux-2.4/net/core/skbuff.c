 ...
 *
 *	Version:	$Id: skbuff.c,v 1.6.214.1 2006/10/16 14:57:39 shmulik Exp $
 *
 *	Fixes:	
...
void __kfree_skb(struct sk_buff *skb)
{
	...
	if (skb->fp_flow_info)
	{
		skb->fp_flow_info->destruct(skb->fp_flow_info);
		skb->fp_flow_info = NULL;
	}
#endif
...
#endif
#ifdef CONFIG_RG_KOS_FEATURE
	n->fp_flow_info = skb->fp_flow_info ?
		skb->fp_flow_info->copy(skb->fp_flow_info) : NULL;
#endif
...
#endif
#ifdef CONFIG_RG_KOS_FEATURE
	new->fp_flow_info = old->fp_flow_info ?
		old->fp_flow_info->copy(old->fp_flow_info) : NULL;
#endif
}
