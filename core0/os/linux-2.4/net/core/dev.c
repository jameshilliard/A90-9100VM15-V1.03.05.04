...
#ifdef CONFIG_RG_FREESWAN
#include <freeswan/klips/net/ipsec/ipsec_encap.h>
#include <freeswan/klips/net/ipsec/ipsec_tunnel.h>
#include <freeswan/klips/net/ipsec/radij.h>
#include <freeswan/klips/net/ipsec/ipsec_radij.h>
#endif
...
static LAN_DEVICE_TABLE_T tblDev, *pTblDev=&tblDev;

/* Prototype declarations. */
int tellabs_CheckDeviceList(struct sk_buff *skb);
static dhcpOptionField *tellabs_option60(unsigned char *pMsg);
#endif
...
#ifdef CONFIG_RG_IPFILTER_MODULE
#define CONFIG_RG_IPFILTER
#endif

static int net_hook_stub(struct sk_buff **skb)
{
    return 1;
}

static int (*net_rx_hook)(struct sk_buff **skb) = net_hook_stub;
static int (*net_tx_hook)(struct sk_buff **skb) = net_hook_stub;

static int net_local_hook_stub(struct sk_buff **skb, int is_local_out)
{
	return 1;
}

int (*net_local_hook)(struct sk_buff **skb, int is_local_out) =
	net_local_hook_stub;
EXPORT_SYMBOL(net_local_hook);

void dev_set_packet_hook(int (*hook)(struct sk_buff **skb), int is_rx)
{
	if (is_rx)
		net_rx_hook = hook ? hook : net_hook_stub;
	else
		net_tx_hook = hook ? hook : net_hook_stub;
}
EXPORT_SYMBOL(dev_set_packet_hook);

void dev_set_local_hook(int (*hook)(struct sk_buff **skb, int is_local_out))
{
	net_local_hook = hook ? hook : net_local_hook_stub;
}
EXPORT_SYMBOL(dev_set_local_hook);


#ifdef CONFIG_VLAN_8021Q_FAST
int (*vlan_rx_hook)(struct sk_buff **skb, struct net_device *dev) = NULL;
int (*vlan_tx_hook)(struct sk_buff **skb, struct net_device *dev) = NULL;
#endif
...
#ifdef CONFIG_RG_FREESWAN
static void dev_fix_by_ipsec_route(struct sk_buff *skb)
{
	struct sockaddr_encap matcher;
	/* The packet must be IP on the ipsec device */
	...
	/* If no route is found, use the physical device */
	if (!ipsec_findroute(&matcher))
	{
	    skb->dev = (struct net_device *)
		(((struct ipsecpriv *)skb->dev->priv)->dev);
	}
}
#endif
...
int dev_queue_xmit(struct sk_buff *skb)
{
...
#ifdef CONFIG_RG_FREESWAN
	/* XXX: hack! for B14501. the problem should be solved by the route
	 * module when it's done for Linux, and then this hack must be
	 * removed. See B13186 */
	if (!strncmp(skb->dev->name, "ipsec", 5))
	    dev_fix_by_ipsec_route(skb);
#endif

	dev = skb->dev;

	/* Lock bh before the hooks in order to prevent RX packets enter the 
	 * hooks while it handles TX packet that is not in bh context */
	...
	return dev_tx_lower(dev, skb);
}

int dev_tx_lower(struct net_device *dev, struct sk_buff *skb)
{
	struct Qdisc  *q;
	
#ifdef CONFIG_VLAN_8021Q_FAST
	if (vlan_tx_hook && vlan_tx_hook(&skb, skb->dev))
	    return 0;
 ...
 *      
 */
void (*netif_rx_led)(struct sk_buff *skb);
EXPORT_SYMBOL(netif_rx_led);
...
int netif_rx(struct sk_buff *skb)
{
	...
	static unsigned long stamp_jiffies;
	...
	{
	    /* update stamp only once a jiffy */
	    if (stamp_jiffies != jiffies)
	    {
		stamp_jiffies = jiffies;
		do_gettimeofday(&stamp);
	    }
	    skb->stamp = stamp;
	}

#ifdef CONFIG_WSTL_SAVE_RXDEV
	if (skb->rx_dev == NULL)
		skb->rx_dev = skb->dev;
#endif
...
int netif_receive_skb(struct sk_buff *skb)
{
...
#ifdef SJH_TELLABS 
    if(tellabs_CheckDeviceList(skb))
    {
        return(0);
    }
#endif
    ...
    rx_dev = skb->dev;
    return net_rx_lower(rx_dev, skb);
}

int net_rx_lower(struct net_device *rx_dev, struct sk_buff *skb)
{
...
void (*netdev_register_led)(struct net_device *dev);
EXPORT_SYMBOL(netdev_register_led);
...
int register_netdevice(struct net_device *dev)
{
	...
	*dp = dev;
	...
	if (netdev_register_led)
		netdev_register_led(dev);
...
#ifdef CONFIG_BCM963XX_COMMON
		queue->filler1 = 0;
		memset(queue->filler2, 0, sizeof(queue->filler2));
#endif
		queue->throttle = 0;
		queue->cng_level = 0;
...
int net_run_sbin_hotplug(struct net_device *dev, char *action)
{
...
EXPORT_SYMBOL(net_run_sbin_hotplug);
#endif

#ifdef SJH_TELLABS
#include <linux/if_vlan.h>

static stbApprovedList stbList = {MAX_NUM_OF_STB_ENTRIES, 1,
{{"UltraLine Series3"}, {"Test STB Default Name_1"}, {"Test STB Default Name_2"},
{"Test STB Default Name_3"}, {"Test STB Default Name_4"}, {"Test STB Default Name_5"},
{"Test STB Default Name_6"}, {"Test STB Default Name_7"}} };

static void tlbsVlan_debugPrint(void *pMsg, int len)
{
    int i;
    unsigned char *pPckt;

    pPckt = (unsigned char *) pMsg;
...
/* Magic cookie setting for all DHCP messages. */
#define COOKIE_LENGTH 4
static unsigned char magicCookie[COOKIE_LENGTH]={0x63,0x82,0x53,0x63};

int tellabs_CheckDeviceList(struct sk_buff *skb)
{
    int i, found=0;
    bootp_pkt *pMsg=NULL;
    dhcpOptionField *pOptionField=NULL, optionField;
    // struct vlan_ethhdr *pVlanHdr=NULL;
    // struct vlan_ethhdr vlanHdr;
    struct net_device *pNetDevice;
    struct ethhdr *pEthHdr, ethHdr;   
    
    if( strcmp(skb->dev->name, "eth0") == 0)
    {
        pMsg = (bootp_pkt *)skb->data;
        
        /* Is it a braodcast */
        /* if( ((skb->mac.ethernet->h_dest[0] == 0xff) && 
             (skb->mac.ethernet->h_dest[1] == 0xff) &&
             (skb->mac.ethernet->h_dest[2] == 0xff) &&
             (skb->mac.ethernet->h_dest[3] == 0xff) &&
             (skb->mac.ethernet->h_dest[4] == 0xff) &&
             (skb->mac.ethernet->h_dest[5] == 0xff)) && 
             (ntohs(skb->mac.ethernet->h_proto) == ETH_P_IP)) */
             
        if( ((skb->mac.ethernet->h_dest[0] & 0x01) == 0x01) && 
             (ntohs(skb->mac.ethernet->h_proto) == ETH_P_IP) )
        {
            /* Check udp source(client) and destination(server) ports. */
            if((ntohs(pMsg->udph.source) == 68) && (ntohs(pMsg->udph.dest) == 67) &&
               (pMsg->op == BOOTP_REQUEST)) 
            {
                /* Search DHCP message for the option-60 field. */  
                pOptionField = tellabs_option60((unsigned char *) pMsg->exten);
             
                /* Did we find an option-60 in DHCP message. */
                if(pOptionField == NULL)  
                {
                    printk("tellabs_CheckDeviceList... no option-60 found\n");
                    return(0);
                }
#if 1                  
                else
                {       
                    memset(&optionField, 0x00, sizeof(dhcpOptionField));
                    
                    memcpy(optionField.vci, pOptionField->vci, pOptionField->length);
                    optionField.length = pOptionField->length;
                    
                    printk("tellabs_CheckDeviceList... VCI-> %s\n", optionField.vci);
                    printk("len-> %d\n", optionField.length);
                }
#endif                
                pNetDevice = dev_get_by_name("eth1");
                if(pNetDevice == NULL)
                {
                    printk("tellabs_CheckDeviceList... ERROR-> can't find eth1\n");
                    return(0);
                }
                
                /* Must be an approved STB vendor, scan default list of STBs 
                   supported by modem. */
                for(i=0; i<MAX_NUM_OF_STB_ENTRIES; i++)
                {
                    if(strncmp(stbList.entry[i].stbName,
                               pOptionField->vci, pOptionField->length) == 0)
                    {
                        /* We have a match. */
                        found = 1;
                        printk("tellabs_CheckDeviceList... STB match found[%d]\n", i);
                    }                   
                }
                
                if(!found)
                {
                    /* Didn't match default list of approved vender's,
                       foward packet to a different VLAN. */
                       
                    printk("tellabs_CheckDeviceList... Not an approved STB vendor[%s]\n", 
                            optionField.vci);
                    return(0);
                }
                                                              
                /* Check to see if this new entry already exists? If so, erase 
                   the old entry. */
                for(i=0; i<MAX_NUM_OF_DEVICE_ENTRIES; i++)
                {
                    if(memcmp(pTblDev->devTbl[i].macAddr, skb->mac.ethernet->h_source, 6) == 0)
                    {
                        memset(&pTblDev->devTbl[i], 0x00, sizeof(DEVICE_SLOT_T));
                        printk("tellabs_CheckDeviceList... erasing old entry devTbl[%d]\n", i);
                    }
                }       
                     
                // printk("tellabs_CheckDeviceList... detect new device\n");

                /* Store MAC address and VCI into a local table. */
                found = 0;
                for(i=0; i<MAX_NUM_OF_DEVICE_ENTRIES; i++)
                {
                    /* Find the first available slot. */
                    if(pTblDev->devTbl[i].state == 0)
                    {
                        pTblDev->devTbl[i].state = 1;
                        memcpy(pTblDev->devTbl[i].macAddr, skb->mac.ethernet->h_source, 6);
                        found = 1;
                        
                        printk("tellabs_CheckDeviceList... new device %02x.%02x.%02x.%02x.%02x.%02x slot[%d]\n",
                                pTblDev->devTbl[i].macAddr[0], pTblDev->devTbl[i].macAddr[1], 
                                pTblDev->devTbl[i].macAddr[2], pTblDev->devTbl[i].macAddr[3], 
                                pTblDev->devTbl[i].macAddr[4], pTblDev->devTbl[i].macAddr[5], i);
                        break;                                
                    }
                }
                ...
                if(!found)
                {
                    printk("tellabs_CheckDeviceList... ERROR->devTbl is full, discard entry\n");
                    return(0);
                }
                
                /* Is there enough headroom in the skbuff for the VLAN header. */
                if(skb_headroom(skb) >= sizeof(struct vlan_ethhdr))
                {
                    /* Space is available, insert VLAN header. */ 
                    // printk("tellabs_CheckDeviceList... available headroom[%d], len[%d]\n", 
                    //        skb_headroom(skb), skb->len);                      
#if 0                    
                    pVlanHdr = (struct vlan_ethhdr *) skb_push(skb, sizeof(struct vlan_ethhdr));
#else
                    printk("tellabs_CheckDeviceList... data len[%d] len[%d] \n", 
                            (unsigned int) skb->data_len, (unsigned int) skb->len);
                    printk("tellabs_CheckDeviceList... pData[%x] pTail[%x] pEnd[%x] pHead[%x] \n",
                           (unsigned int) skb->data, (unsigned int) skb->tail, 
                           (unsigned int) skb->end, (unsigned int) skb->head);
                           
                    pEthHdr = (struct ethhdr *) skb_push(skb, sizeof(struct ethhdr));
#endif         
                }
                else
                {
                    printk("tellabs_CheckDeviceList... ERROR can't fit new hdr-> %d\n", 
                            skb_headroom(skb));
                    kfree_skb(skb);
                    return(0);
                }
 #if 0               
                /* Reformat packet to include VLAN tag. */
                memcpy(vlanHdr.h_dest, skb->mac.ethernet->h_dest, 6);                
                memcpy(vlanHdr.h_source, skb->mac.ethernet->h_source, 6);
                vlanHdr.h_vlan_proto = htons(0x8100);

                /* Temp. value until specified by Tellabs -> 12 bit value. 
                   May want to use the port number on the switch.... */
                vlanHdr.h_vlan_TCI = htons(0x0FFF & 0x0D0D);                 
                vlanHdr.h_vlan_encapsulated_proto = skb->mac.ethernet->h_proto;
                
                memcpy(pVlanHdr, &vlanHdr, sizeof(struct vlan_ethhdr));
#else                                          
                memcpy(ethHdr.h_dest, skb->mac.ethernet->h_dest, ETH_ALEN);                
                memcpy(ethHdr.h_source, skb->mac.ethernet->h_source, ETH_ALEN);
                ethHdr.h_proto = htons(0x0800);
                
                memcpy(pEthHdr, &ethHdr, sizeof(struct ethhdr));

                printk("tellabs_CheckDeviceList... data len[%d] len[%d] \n", 
                        (unsigned int) skb->data_len, skb->len);
                printk("tellabs_CheckDeviceList... pData[%x] pTail[%x] pEnd[%x] pHead[%x] \n",
                       (unsigned int) skb->data, (unsigned int) skb->tail, 
                       (unsigned int) skb->end, (unsigned int) skb->head);
#endif

                tlbsVlan_debugPrint((void *)skb->data, 30);
                
                /* Forward packet to WAN interface. */
                skb->dev = pNetDevice;

                if(dev_queue_xmit(skb) != 0)
                {
                    printk("tellabs_CheckDeviceList... ERROR-> fail dev_queue_xmit\n");
                }
                else
                {
                    printk("tellabs_CheckDeviceList... sending DHCP pkt\n");
                }
                return(1);             
            }
        }
    ...
    {
        tlbsVlan_debugPrint((void *)skb->data, 16);
        /* Do nothing for now.... */
    }
...
static dhcpOptionField *tellabs_option60(unsigned char *pMsg)
{
    unsigned char *pData;
    int retVal, exit=0;
    dhcpOptionField *pOpt=NULL;
    
    /* Start at the beginning of option fields in message. 
       Skipping over the magic cookie. */
    retVal = memcmp(pMsg, magicCookie, COOKIE_LENGTH);
    if(retVal == 0)
    {
        // tlbsVlan_debugPrint(pMsg, 64);
        pData = pMsg + 4;
        
        do{
            // printk("tellabs_option60... option->%d\n", *pData);
                 
            /* We have a valid cookie, seach for option-60. */
            if(*pData != 60)
            {
                if(*pData == 0xff)
                {
                    /* End of message, couldn't find option 60. */
                    exit=1;
                }
                else
                {
                    /* Skip to the next option field. */
                    pData++;
                    pData += ((*pData) + 1);
                }
            }
