 ...
 *
 *	Version: $Id: igmp.c,v 1.6.2.1.10.3 2007/02/04 14:44:05 shmulik Exp $
 *
 *	Authors:
    ...
    int type, u8 respond, u32 dst, struct rtable **rt)
{
	...
	skb=alloc_skb(igmp_sz+dev->hard_header_len+15, GFP_ATOMIC);
	if (skb == NULL) {
		ip_rt_put(*rt);
		return NULL;
	}
...
static void igmpv3_add_delrec(struct in_device *in_dev, struct ip_mc_list *im)
{
	 ...
	 */
	pmc = (struct ip_mc_list *)kmalloc(sizeof(*pmc), GFP_ATOMIC);
	if (!pmc)
		return;
...
void ip_mc_inc_group(struct in_device *in_dev, u32 addr)
{
	...
	im = (struct ip_mc_list *)kmalloc(sizeof(*im), GFP_ATOMIC);
	if (!im)
		goto out;
