 ...
 *		The IP to API glue.
 *		
 * Version:	$Id: ip_sockglue.c,v 1.8.34.1.10.1 2006/12/18 15:39:04 lena Exp $
 *
 * Authors:	see ip.c
 *
 * Fixes:
...
int (*igmp_proxy_setsockopt)(struct sock *sk, int optname, char *optval,
	int optlen);
int (*igmp_proxy_getsockopt)(struct sock *sk, int optname, char *optval,
	int *optlen);
EXPORT_SYMBOL(igmp_proxy_setsockopt);
EXPORT_SYMBOL(igmp_proxy_getsockopt);
...
int ip_setsockopt(struct sock *sk, int level, int optname, char *optval, int optlen)
{
	...
	if (optname >= IGMP_PROXY_BASE && optname <= IGMP_PROXY_BASE + 10)
		return igmprx_setsockopt(sk, optname, optval, optlen);
#endif
#ifdef CONFIG_RG_IGMP_PROXY_MODULE
	if (igmp_proxy_setsockopt &&
		optname >= IGMP_PROXY_BASE && optname <= IGMP_PROXY_BASE + 10)
	{
		return igmp_proxy_setsockopt(sk, optname, optval, optlen);
	}
#endif
	...
	switch (optname) {
		...
		case IP_TOS:	/* This sets both TOS and Precedence */
			...
			if (sk->protinfo.af_inet.tos != val) {
				...
				sk->priority = rt_dscp2priority(val);
				sk_dst_reset(sk); 
			}
			break;
		case IP_TTL:
...
#ifdef CONFIG_RG_IGMP_PROXY
	if (optname >= IGMP_PROXY_BASE && optname <= IGMP_PROXY_BASE + 10)
		return igmprx_getsockopt(sk, optname, optval, optlen);
#endif
#ifdef CONFIG_RG_IGMP_PROXY_MODULE
	if (igmp_proxy_getsockopt &&
		optname >= IGMP_PROXY_BASE && optname <= IGMP_PROXY_BASE + 10)
	{
		return igmp_proxy_getsockopt(sk, optname, optval, optlen);
	}
#endif
