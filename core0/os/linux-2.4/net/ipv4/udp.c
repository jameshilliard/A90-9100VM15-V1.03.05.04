 ...
 *
 * Version:	$Id: udp.c,v 1.7.12.2 2006/12/18 15:39:04 lena Exp $
 *
...
int (*udp_espinudp_encap)(struct sk_buff *skb, struct sock *sk);

void udp_espinudp_encap_set(int (*func)(struct sk_buff *skb, struct sock *sk))
{
    udp_espinudp_encap = func;
}
EXPORT_SYMBOL(udp_espinudp_encap_set);
#endif
...
#ifdef CONFIG_IPSEC_NAT_TRAVERSAL
	if (sk->tp_pinfo.af_udp.esp_in_udp && udp_espinudp_encap &&
	    udp_espinudp_encap(skb, sk) == 0) {
