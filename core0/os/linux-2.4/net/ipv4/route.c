 ...
 *
 * Version:	$Id: route.c,v 1.7.46.1 2006/12/18 15:39:04 lena Exp $
 *
...
int (*igmp_proxy_input)(struct sk_buff *skb);
EXPORT_SYMBOL(igmp_proxy_input);
EXPORT_SYMBOL(ip_rt_multicast_event);
#endif
...
/* Since Jungo implement routing, based on DSCP field, I just comment this code,
				...
				u8 tos, struct net_device *dev, int our)
{
	...
	if (IN_DEV_MFORWARD(in_dev))
		rth->u.dst.input = igmprx_input;
#endif
#ifdef CONFIG_RG_IGMP_PROXY_MODULE
	if (igmp_proxy_input && IN_DEV_MFORWARD(in_dev))
		rth->u.dst.input = igmp_proxy_input;
#endif
...
#endif
#ifdef CONFIG_RG_IGMP_PROXY_MODULE
			    || (IN_DEV_MFORWARD(in_dev) && igmp_proxy_input)
#endif
			    ) {
...
#endif
#ifdef CONFIG_RG_IGMP_PROXY_MODULE
		if (res.type == RTN_MULTICAST && igmp_proxy_input)
		{
			struct in_device *in_dev = in_dev_get(dev_out);
			if (in_dev) {
				if (IN_DEV_MFORWARD(in_dev)) {
