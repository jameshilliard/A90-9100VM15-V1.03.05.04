...
/* USB LED Interface Stuff */
static int num_active_interfaces=0;
static void (*set_device_present_cb)(void) = NULL;
static void (*clear_device_present_cb)(void) = NULL;
static void (*activity_cb)(void) = NULL;
void usbled_register_callbacks(void (*set_device_present)(void), void (*clear_device_present)(void), void (*activity)(void) ) {
    set_device_present_cb = set_device_present;
    clear_device_present_cb = clear_device_present;
	...
	for (ifnum = 0; ifnum < dev->actconfig->bNumInterfaces; ifnum++) {
		...
		if (!usb_interface_claimed(dev->actconfig->interface + ifnum)) {
			if (usb_find_interface_driver(dev, ifnum))
			    ...
			    claimed++;
			    if (++num_active_interfaces==2) {  /* The first one is the "root hub" - the 2nd one is an external device. */
				/* Notify callback function of USB device present and claimed */
				if (set_device_present_cb != NULL) {
				    set_device_present_cb();
				}
...
struct urb *usb_alloc_urb(int iso_packets)
{
	...
	/* Notify callback function of USB activity */
	if (activity_cb != NULL) {
	    activity_cb();
	...
	if (dev->actconfig) {
		for (i = 0; i < dev->actconfig->bNumInterfaces; i++) {
			...
			if (driver) {
				...
				if (--num_active_interfaces==1) {  /* When there's only one device, it's the root hub, not an external device. */
				/* Notify callback function that no more devices present and claimed */
				    if (clear_device_present_cb != NULL) {
					clear_device_present_cb();
				    }
...
#ifdef CONFIG_ARCH_IXP425
	/* XXX This code is not supposed to be here,
	 * but required for USB 1.1 devices after USB 2.0 hubs,
	 * we don't know why
	 * XXX */
	invalidate_dcache_range((__u32)buf, (__u32)buf + size);
#endif
...
EXPORT_SYMBOL(usbled_register_callbacks);
