 ...
 *
 * $Id: usb.c,v 1.8 2004/05/02 12:07:50 alex Exp $
 *
...
static void storage_disconnect(struct usb_device *dev, void *ptr)
{
	...
	spin_lock_irqsave(&(ss->queue_exclusion), flags);
	if (ss->queue_srb) {
		ss->queue_srb->result = DID_ERROR << 16;
		ss->queue_srb->scsi_done(ss->queue_srb);
		ss->queue_srb = NULL;
		ss->action = 0;
	}
	spin_unlock_irqrestore(&(ss->queue_exclusion), flags);
	...
	for (i = 0; i < 10; i++) {
		if (!scsi_unregister_module(MODULE_SCSI_HA, &(ss->htmplt)))
			break;
		printk("storage/usb.c: scsi_unregister_module() failed try %d\n", i);
	...
	if (i == 10)
	{
		printk("storage/usb.c: Failed to remove USB-storage SCSI host\n");
		return;
	}
...
/*This portion here for debugging. We don't need so we need to remove this or just make sure LOCK_ALL 
is always 1*/
#define LOCK_ALL 1
#if !(LOCK_ALL)
struct us_data *get_us_list (int host_no)
{
    struct us_data *us;
...
int get_usb_storage_us_info ( int host_no, char ** buff, int buff_size)
{
    ...
    #if !(LOCK_ALL)
    us = get_us_list(host_no);
    #endif

    if (!us) {
        //printk("inside get_usb_storage_us_info: us is NULL\n");
        #if LOCK_ALL
        up(&us_list_semaphore);
        #endif
        return 1;
   ...
   // printk("inside get_usb_storage_us_info: we found the US \n");
    char temp_buff[SCSI_IOCTL_GET_GUID_BUFFER_LEN];
    memset (temp_buff, 0, sizeof (temp_buff) );
    
    snprintf(temp_buff, sizeof(temp_buff), " "GUID_FORMAT" ", GUID_ARGS(us->guid));

   // printk("inside get_usb_storage_us_info: GUID is  %s and sizeof *buff is %d\n", temp_buff, buff_size);
    if (buff && *buff) {
       memcpy (*buff, temp_buff, buff_size);
    }
