 ...
 *
 * $Id: scsiglue.c,v 1.5 2004/05/02 12:07:50 alex Exp $
 *
...
//add ioctl call here
int scsiglue_ioctl(Scsi_Device * dev, int cmd, void *arg)
{
	//printk("inside ioctl in usb storage top line\n");
	switch (cmd) {
    case SCSI_IOCTL_GET_GUID:
        {
        
        char buff[SCSI_IOCTL_GET_GUID_BUFFER_LEN];
        char *ptr = buff;
        buff[SCSI_IOCTL_GET_GUID_BUFFER_LEN - 1] = 0;
        if ( get_usb_storage_us_info ( dev->host->host_no, &ptr, (SCSI_IOCTL_GET_GUID_BUFFER_LEN - 1) ) )
            return -EINVAL;
        else{
            int len = strlen(buff);
            if ( (len > SCSI_IOCTL_GET_GUID_BUFFER_LEN) || (len < 0) ) {
                 //printk("inside ioctl something wrong len has invalid size\n");
                 len = SCSI_IOCTL_GET_GUID_BUFFER_LEN -1;
            }
            buff[len] = 0;
            copy_to_user(arg, buff, len + 1);
